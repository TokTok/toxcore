...
{
    if (argc < 4) {
        printf("Usage: ./save-generator <name> <number of friends> <friend id> ...\n");
        return -1;
    }
    ...
    for (int i = 3; i < argc; i++) { //start at 3 because that is where the tox ids are
        ...
        if (friend_err != TOX_ERR_FRIEND_ADD_OK) {
            printf("Failed to add friend number %u. Error number: %u\n", i, friend_err);
        }
    }
