...
void idle_cycle(Peer *peers, int peercount, void *userdata)
{
    int j;
...
void idle_n_secs(int n, Peer *peers, int peercount, void *userdata)
{
    int i;
...
void basicannouncetest()
{
    ...
    for (i = 0; i < PEERCOUNT; i++) {
        ...
        peers[i].tox = new_messenger(NULL, &options, 0);
    ...
    for (;;) {
        idle_cycle(peers, PEERCOUNT, NULL);
    ...
    idle_n_secs(10, peers, PEERCOUNT, NULL);
