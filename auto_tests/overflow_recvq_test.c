...
static void handle_friend_message(Tox *tox, uint32_t friend_number,
...
static void net_crypto_overflow_test(AutoTox *autotoxes) {
  ...
  for (uint32_t tox_index = 1; tox_index < 3; tox_index++) {
    for (uint32_t i = 0; i < NUM_MSGS; i++) {
      ...
      tox_friend_send_message(autotoxes[tox_index].tox, 0,
                              TOX_MESSAGE_TYPE_NORMAL, message, sizeof message,
                              &err);
      ...
      ck_assert_msg(err == TOX_ERR_FRIEND_SEND_MESSAGE_OK,
                    "tox%u failed to send message number %u: %d", tox_index, i,
                    err);
    }
  }
  ...
  printf("tox%u received %u messages\n", autotoxes[0].index,
         ((State *)autotoxes[0].state)->recv_count);
}

int main(void) {
  setvbuf(stdout, nullptr, _IONBF, 0);

  run_auto_test(3, net_crypto_overflow_test, sizeof(State),
