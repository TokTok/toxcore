...
static void net_crypto_overflow_test(AutoTox *autotoxes) {
  ...
  for (uint32_t i = 0; i < NUM_MSGS; i++) {
    ...
    tox_friend_send_message(autotoxes[0].tox, 0, TOX_MESSAGE_TYPE_NORMAL,
                            message, sizeof message, &err);
  ...
  ck_assert_msg(errored,
                "expected SENDQ error at some point (increase NUM_MSGS?)");
}
