...
static void audio_callback(void *tox, uint32_t groupnumber, uint32_t peernumber,
                           const int16_t *pcm, unsigned int samples,
...
static void handle_conference_invite(Tox *tox, uint32_t friendnumber,
                                     ...
                                     void *user_data) {
  const AutoTox *autotox = (AutoTox *)user_data;
  ck_assert_msg(type == TOX_CONFERENCE_TYPE_AV,
                "tox #%u: wrong conference type: %d", autotox->index, type);

  ck_assert_msg(toxav_join_av_groupchat(tox, friendnumber, data, length,
                                        audio_callback, user_data) == 0,
...
static bool all_got_audio(AutoTox *autotoxes, const bool *disabled) {
  ...
  for (uint32_t i = 0; i < NUM_AV_GROUP_TOX; ++i) {
    ...
    if (disabled[i] ^
        (state->received_audio_num != NUM_AV_GROUP_TOX - num_disabled - 1)) {
      return false;
    }
  }
...
static bool test_audio(AutoTox *autotoxes, const bool *disabled, bool quiet) {
  ...
  for (uint32_t n = 0; n < GROUP_AV_AUDIO_ITERATIONS; n++) {
    for (uint32_t i = 0; i < NUM_AV_GROUP_TOX; ++i) {
      ...
      if (toxav_group_send_audio(autotoxes[i].tox, 0, PCM,
                                 GROUP_AV_TEST_SAMPLES, 1, 48000) != 0) {
        if (!quiet) {
...
static void test_eventual_audio(AutoTox *autotoxes, const bool *disabled,
                                uint64_t timeout) {
...
static void do_audio(AutoTox *autotoxes, uint32_t iterations) {
  ...
  for (uint32_t f = 0; f < iterations; ++f) {
    for (uint32_t i = 0; i < NUM_AV_GROUP_TOX; ++i) {
      ck_assert_msg(toxav_group_send_audio(autotoxes[i].tox, 0, PCM,
                                           GROUP_AV_TEST_SAMPLES, 1,
                                           48000) == 0,
...
#define JITTER_SETTLE_TIME                                                     \
  (GROUP_JBUF_DEAD_SECONDS * 1000 +                                            \
   NUM_AV_GROUP_TOX * ITERATION_INTERVAL * (GROUP_AV_AUDIO_ITERATIONS + 1))
...
static void run_conference_tests(AutoTox *autotoxes) {
  ...
  for (uint32_t i = 0; i < NUM_AV_GROUP_TOX; ++i) {
    if (restarting[i]) {
      ...
      ck_assert_msg(toxav_groupchat_enable_av(autotoxes[i].tox, 0,
                                              audio_callback,
                                              &autotoxes[i]) == 0,
                    "#%u failed to re-enable av", autotoxes[i].index);
      ck_assert_msg(toxav_groupchat_av_enabled(autotoxes[i].tox, 0),
                    "#%u av not enabled even after enabling",
                    autotoxes[i].index);
    }
  }
   ...
   * looser condition). */
  test_eventual_audio(autotoxes, disabled,
                      JITTER_SETTLE_TIME + NUM_AV_GROUP_TOX * 1000);
  ...
  for (uint32_t i = 0; i < NUM_AV_DISABLE; ++i) {
    ...
    ck_assert_msg(toxav_groupchat_enable_av(autotoxes[disable].tox, 0,
                                            audio_callback,
                                            &autotoxes[disable]) != 0,
  ...
  for (uint32_t i = 0; i < NUM_AV_DISABLE; ++i) {
    ...
    ck_assert_msg(toxav_groupchat_enable_av(autotoxes[i].tox, 0, audio_callback,
                                            &autotoxes[i]) == 0,
                  "#%u failed to re-enable av", autotoxes[i].index);
  }
...
static void test_groupav(AutoTox *autotoxes) {
  ...
  ck_assert_msg(toxav_add_av_groupchat(autotoxes[0].tox, audio_callback,
                                       &autotoxes[0]) != UINT32_MAX,
