...
START_TEST(test_client)
{
    ...
    ck_assert_msg(tcp_con_status(conn) == TCP_CLIENT_UNCONFIRMED, "Wrong status. Expected: %u, is: %u", TCP_CLIENT_UNCONFIRMED,
                  tcp_con_status(conn));
    c_sleep(50);
    do_TCP_server(tcp_s);
    c_sleep(50);
...
START_TEST(test_client_invalid)
{
    ...
    c_sleep(50);
    do_TCP_connection(conn, NULL);
    ck_assert_msg(tcp_con_status(conn) == TCP_CLIENT_CONNECTING, "Wrong status. Expected: %u, is: %u", TCP_CLIENT_CONNECTING,
                  tcp_con_status(conn));
    c_sleep(5000);
    do_TCP_connection(conn, NULL);
    ck_assert_msg(tcp_con_status(conn) == TCP_CLIENT_CONNECTING, "Wrong status. Expected: %u, is: %u", TCP_CLIENT_CONNECTING,
    ...
    c_sleep(6000);
    do_TCP_connection(conn, NULL);
    ck_assert_msg(tcp_con_status(conn) == TCP_CLIENT_DISCONNECTED, "Wrong status. Expected: %u, is: %u", TCP_CLIENT_DISCONNECTED,
                  tcp_con_status(conn));
