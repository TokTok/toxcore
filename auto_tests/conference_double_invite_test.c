    ...
    const uint8_t *cookie, size_t length, void *user_data)
{
    ...
    if (friend_number != -1) {
        ...
        ck_assert_msg(err == TOX_ERR_CONFERENCE_JOIN_OK, "could not join group");
...
static void conference_double_invite_test(Tox **toxes, State *state)
{
...

    {
        ...
        state[0].joined = true;
        ck_assert_msg(err == TOX_ERR_CONFERENCE_NEW_OK, "could not create group");
...

    {
        ...
        ck_assert_msg(err == TOX_ERR_CONFERENCE_INVITE_OK, "could not invite friend");
