...
bool all_connected(uint32_t tox_count, AutoTox *autotoxes) {
  for (uint32_t i = 0; i < tox_count; i++) {
    if (tox_self_get_connection_status(autotoxes[i].tox) ==
        TOX_CONNECTION_NONE) {
      return false;
    }
  }
...
bool all_friends_connected(uint32_t tox_count, AutoTox *autotoxes) {
  for (uint32_t i = 0; i < tox_count; i++) {
    ...
    for (size_t j = 0; j < friend_count; j++) {
      if (tox_friend_get_connection_status(autotoxes[i].tox, j, nullptr) ==
          TOX_CONNECTION_NONE) {
        return false;
      }
    }
...
static uint64_t get_state_clock_callback(Mono_Time *mono_time,
                                         void *user_data) {
...
void set_mono_time_callback(AutoTox *tox) {
  ...
  mono_time_set_current_time_callback(mono_time, get_state_clock_callback,
                                      &tox->clock);
}
...
void save(AutoTox *autotox) {
  ...
  ck_assert_msg(autotox->save_size > 0, "save is invalid size %u",
                (unsigned)autotox->save_size);
...
void reload(AutoTox *autotox) {
  ...
  tox_options_set_savedata_data(options, autotox->save_state,
                                autotox->save_size);
...
void run_auto_test(uint32_t tox_count, void test(AutoTox *autotoxes),
  ...
  if (options->graph == GRAPH_LINEAR) {
    ...
    for (uint32_t i = options->tcp_relays; i < tox_count; i++) {
      for (uint32_t j = i - 1; j != i + 3; j += 2) {
        ...
        tox_friend_add_norequest(autotoxes[i].tox, public_key, nullptr);
      }
    }
  ...
  if (options->tcp_relays) {
    ...
    for (uint32_t i = 0; i < tox_count; i++) {
      ...
      ck_assert_msg(tox_add_tcp_relay(autotoxes[i].tox, "localhost",
                                      options->tcp_first_port + relay, dht_key,
                                      &error),
  ...
  for (uint32_t i = 1; i < tox_count; i++) {
    tox_bootstrap(autotoxes[i].tox, "localhost", dht_port, dht_key, nullptr);
  }
