...
static void test_few_clients(void)
{
    ...
    while (1) {
        ...
        if (tox_self_get_connection_status(tox1) && tox_self_get_connection_status(tox2)
            ...
            if (off) {
                printf("Toxes are online, took %lld seconds\n", time(nullptr) - cur_time);
    ...
    printf("tox clients connected took %lld seconds\n", time(nullptr) - con_time);
    ...
    while (1) {
        ...
        if (tox_self_get_connection_status(tox1) && tox_self_get_connection_status(tox2)
                && tox_self_get_connection_status(tox3)) {
            if (off) {
                printf("Toxes are online again after reloading, took %lld seconds\n", time(nullptr) - cur_time);
                con_time = time(nullptr);
                off = 0;
            }
    ...
    printf("test_few_clients succeeded, took %lld seconds\n", time(nullptr) - cur_time);
