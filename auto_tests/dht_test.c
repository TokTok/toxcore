...
#define PACKAGE_SIZE (SIZE_IPPORT + CRYPTO_PUBLIC_KEY_SIZE)
#define MAX_COUNT 3
...
static void dht_pack_unpack(const Node_format *nodes, size_t size, uint8_t *data, size_t length)
{
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        ck_assert_msg(ipp1->port == ipp2->port, "Unsucessful port unpack");

        const uint8_t *pk1 = nodes[i].public_key;
        const uint8_t *pk2 = nodes_unpacked[i].public_key;
        ck_assert_msg(!memcmp(pk1, pk2, CRYPTO_PUBLIC_KEY_SIZE), "Unsucessful pk unpack");
    }
}
...
START_TEST(test_dht_package)
{
    const uint16_t length = MAX_COUNT * PACKAGE_SIZE;
