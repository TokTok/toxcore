...
static void handle_conference_invite(Tox *tox, uint32_t friend_number,
  ...
  ck_assert_msg(err == TOX_ERR_CONFERENCE_JOIN_OK,
                "attempting to join the conference returned with an error: %d",
                err);
...
static void handle_peer_list_changed(Tox *tox, uint32_t conference_number,
                                     void *user_data) {
  const AutoTox *autotox = (AutoTox *)user_data;
  State *state = (State *)autotox->state;
  ...
  uint32_t const count =
      tox_conference_peer_count(tox, conference_number, &err);
...
static void rebuild_peer_list(Tox *tox) {
  for (uint32_t conference_number = 0;
    ...
    ck_assert_msg(
        ...
        conference_number, err);

    for (uint32_t peer_number = 0; peer_number < count; peer_number++) {
      size_t size = tox_conference_peer_get_name_size(tox, conference_number,
                                                      peer_number, &err);
      ck_assert_msg(err == TOX_ERR_CONFERENCE_PEER_QUERY_OK,
                    "failed to get conference peer %u's name size (conference "
                    "= %u): err = %d",
                    peer_number, conference_number, err);

      uint8_t *const name = (uint8_t *)malloc(size);
      tox_conference_peer_get_name(tox, conference_number, peer_number, name,
                                   &err);
      ck_assert_msg(
          err == TOX_ERR_CONFERENCE_PEER_QUERY_OK,
...
static void conference_peer_nick_test(AutoTox *autotoxes) {
  ...
  tox_callback_conference_peer_list_changed(autotoxes[0].tox,
                                            handle_peer_list_changed);
  tox_callback_conference_peer_list_changed(autotoxes[1].tox,
                                            handle_peer_list_changed);

  // Set the names of the toxes.
  tox_self_set_name(autotoxes[0].tox, (const uint8_t *)"test-tox-0", 10,
                    nullptr);
  tox_self_set_name(autotoxes[1].tox, (const uint8_t *)"test-tox-1", 10,
                    nullptr);
...

  {
    ...
    state[0]->joined = true;
    ck_assert_msg(
        err == TOX_ERR_CONFERENCE_NEW_OK,
        "attempting to create a new conference returned with an error: %d",
        err);
...

  {
    ...
    ck_assert_msg(err == TOX_ERR_CONFERENCE_INVITE_OK,
                  "attempting to invite a friend returned with an error: %d",
                  err);
    fprintf(stderr, "tox0 invited tox1\n");
  }

  fprintf(stderr,
          "Waiting for invitation to arrive and peers to be in the group\n");

  do {
    iterate_all_wait(2, autotoxes, ITERATION_INTERVAL);
  } while (!state[0]->joined || !state[1]->joined ||
           !state[0]->friend_in_group || !state[1]->friend_in_group);
...
int main(void) {
  ...
  run_auto_test(2, conference_peer_nick_test, sizeof(State),
