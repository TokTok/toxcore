...
static void handle_lossy_packet(Tox *tox, uint32_t friend_number,
...
static void test_lossy_packet(AutoTox *autotoxes) {
  ...
  bool ret = tox_friend_send_lossy_packet(autotoxes[0].tox, 0, packet,
                                          sizeof(packet), nullptr);
  ck_assert_msg(ret == false,
                "should not be able to send custom packets this big %i", ret);

  ret = tox_friend_send_lossy_packet(autotoxes[0].tox, 0, packet,
