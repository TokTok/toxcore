...
static void handle_conference_invite(Tox *tox, uint32_t friend_number,
  ...
  if (friend_number != -1) {
    ...
    ck_assert_msg(
        err == TOX_ERR_CONFERENCE_JOIN_OK,
...
static void wait_connected(AutoTox *autotoxes, AutoTox *autotox,
                           uint32_t friendnumber) {
  do {
    iterate_all_wait(NUM_INVITE_MERGE_TOX, autotoxes, ITERATION_INTERVAL);
  } while (tox_friend_get_connection_status(autotox->tox, friendnumber,
                                            nullptr) == TOX_CONNECTION_NONE);
}

static void do_invite(AutoTox *autotoxes, AutoTox *inviter, AutoTox *invitee,
                      uint32_t friendnum) {
  fprintf(stderr, "#%u inviting #%u\n", inviter->index, invitee->index);

  Tox_Err_Conference_Invite err;
  tox_conference_invite(inviter->tox, friendnum,
                        ((State *)inviter->state)->conference, &err);
  ck_assert_msg(err == TOX_ERR_CONFERENCE_INVITE_OK,
                "#%u attempting to invite #%u (friendnumber %u) returned with "
                "an error: %d",
                inviter->index, invitee->index, friendnum, err);
...
static bool group_complete(AutoTox *autotoxes) {
  ...
  for (int i = 0; i < NUM_INVITE_MERGE_TOX; i++) {
    ...
    const int ct = tox_conference_peer_count(
        autotoxes[i].tox, ((State *)autotoxes[i].state)->conference, nullptr);
...
static void conference_invite_merge_test(AutoTox *autotoxes) {
  ...
  do {
    iterate_all_wait(NUM_INVITE_MERGE_TOX, autotoxes, ITERATION_INTERVAL);
  } while (tox_conference_peer_count(autotoxes[2].tox, state2->conference,
                                     nullptr) != 1);
...
int main(void) {
  ...
  run_auto_test(NUM_INVITE_MERGE_TOX, conference_invite_merge_test,
