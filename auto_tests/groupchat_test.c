...
START_TEST(test_text_all)
{
    ...
    for (i = 1; i < NUM_TOXES; ++i) {
        ...
        tox_self_set_name(toxes[i], (const uint8_t *)name, strlen(name), NULL);
    }
    ...
    uint32_t groupnum = tox_group_new(toxes[1], TOX_GROUP_PRIVACY_STATE_PUBLIC, (const uint8_t *)"test", 4, &new_err);
