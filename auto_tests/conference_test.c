...
static void handle_self_connection_status(Tox *tox,
...
static void handle_friend_connection_status(Tox *tox, uint32_t friendnumber,
                                            Tox_Connection connection_status,
                                            void *user_data) {
  const AutoTox *autotox = (AutoTox *)user_data;
...
static void handle_conference_invite(Tox *tox, uint32_t friendnumber,
                                     Tox_Conference_Type type,
                                     const uint8_t *data, size_t length,
                                     void *user_data) {
  const AutoTox *autotox = (AutoTox *)user_data;
  ck_assert_msg(type == TOX_CONFERENCE_TYPE_TEXT,
                "tox #%u: wrong conference type: %d", autotox->index, type);

  Tox_Err_Conference_Join err;
  uint32_t g_num = tox_conference_join(tox, friendnumber, data, length, &err);

  ck_assert_msg(err == TOX_ERR_CONFERENCE_JOIN_OK,
                "tox #%u: error joining group: %d", autotox->index, err);
  ...
  ck_assert_msg(err != TOX_ERR_CONFERENCE_JOIN_OK,
                "tox #%u: joining groupchat twice should be impossible.",
                autotox->index);
}

static void handle_conference_connected(Tox *tox, uint32_t conference_number,
                                        void *user_data) {
  const AutoTox *autotox = (AutoTox *)user_data;
  ...
  ck_assert_msg(err == TOX_ERR_CONFERENCE_INVITE_OK,
                "tox #%u failed to invite next friend: err = %d",
                autotox->index, err);
...
static void handle_conference_message(Tox *tox, uint32_t groupnumber,
                                      uint32_t peernumber,
                                      Tox_Message_Type type,
                                      const uint8_t *message, size_t length,
                                      void *user_data) {
  if (length == (sizeof(GROUP_MESSAGE) - 1) &&
      memcmp(message, GROUP_MESSAGE, sizeof(GROUP_MESSAGE) - 1) == 0) {
    ++num_recv;
  }
}

static bool toxes_are_disconnected_from_group(uint32_t tox_count,
                                              AutoTox *autotoxes,
                                              bool *disconnected) {
  ...
  for (uint32_t i = 0; i < tox_count; i++) {
    ...
    if (tox_conference_peer_count(autotoxes[i].tox, 0, nullptr) >
        tox_count - num_disconnected) {
      return false;
    }
  }
...
static void disconnect_toxes(uint32_t tox_count, AutoTox *autotoxes,
  ...
  do {
    ...
    do {
      ...
      c_sleep(20);
    } while (!toxes_are_disconnected_from_group(tox_count, autotoxes,
                                                disconnect_now));
...
static bool names_propagated(uint32_t tox_count, AutoTox *autotoxes) {
  for (uint32_t i = 0; i < tox_count; ++i) {
    for (uint32_t j = 0; j < tox_count; ++j) {
      const size_t len =
          tox_conference_peer_get_name_size(autotoxes[i].tox, 0, j, nullptr);
...
static void run_conference_tests(AutoTox *autotoxes) {
  ...
  for (uint16_t i = 0; i < NUM_GROUP_TOX; ++i) {
    ...
    ck_assert_msg(err == TOX_ERR_CONFERENCE_SET_MAX_OFFLINE_OK,
                  "tox #%u failed to set max offline: err = %d",
                  autotoxes[i].index, err);
  }
  ...
  if (check_name_change_propagation) {
    ...
    for (uint32_t i = 0; i < NUM_GROUP_TOX; ++i) {
      ...
      tox_self_set_name(autotoxes[i].tox, (const uint8_t *)name, NAMELEN,
                        nullptr);
    }
  }

  for (uint16_t i = 0; i < NUM_GROUP_TOX; ++i) {
    const uint32_t num_frozen =
        tox_conference_offline_peer_count(autotoxes[i].tox, 0, nullptr);
  ...
  for (uint32_t i = 0; i < NUM_GROUP_TOX; ++i) {
    tox_callback_conference_message(autotoxes[i].tox,
                                    &handle_conference_message);

    iterate_all_wait(NUM_GROUP_TOX, autotoxes, ITERATION_INTERVAL);
  }

  Tox_Err_Conference_Send_Message err;
  ck_assert_msg(tox_conference_send_message(
                    autotoxes[random_u32() % NUM_GROUP_TOX].tox, 0,
                    TOX_MESSAGE_TYPE_NORMAL, (const uint8_t *)GROUP_MESSAGE,
                    sizeof(GROUP_MESSAGE) - 1, &err) != 0,
    ...
    for (uint32_t i = 0; i < NUM_GROUP_TOX; ++i) {
      for (uint32_t j = 0; j < NUM_GROUP_TOX; ++j) {
        ...
        ck_assert_msg(memcmp(name, "New", 3) == 0,
                      "name of #%u according to #%u not updated",
                      autotoxes[j].index, autotoxes[i].index);
      }
    }
  }
  ...
  for (uint32_t k = NUM_GROUP_TOX; k != 0; --k) {
    ...
    for (uint32_t i = 0; i < k - 1; ++i) {
      uint32_t peer_count =
          tox_conference_peer_count(autotoxes[i].tox, 0, nullptr);
      ck_assert_msg(
          peer_count == (k - 1),
...
static void test_many_group(AutoTox *autotoxes) {
  ...
  for (uint32_t i = 0; i < NUM_GROUP_TOX; ++i) {
    tox_callback_self_connection_status(autotoxes[i].tox,
                                        &handle_self_connection_status);
    tox_callback_friend_connection_status(autotoxes[i].tox,
                                          &handle_friend_connection_status);
    tox_callback_conference_invite(autotoxes[i].tox, &handle_conference_invite);
    tox_callback_conference_connected(autotoxes[i].tox,
                                      &handle_conference_connected);
  ...
  ck_assert_msg(tox_conference_new(autotoxes[0].tox, nullptr) != UINT32_MAX,
                "failed to create group");
  printf("tox #%u: inviting its first friend\n", autotoxes[0].index);
  ck_assert_msg(tox_conference_invite(autotoxes[0].tox, 0, 0, nullptr) != 0,
                "failed to invite friend");
  ((State *)autotoxes[0].state)->invited_next = true;
  ck_assert_msg(tox_conference_set_title(autotoxes[0].tox, 0,
                                         (const uint8_t *)"Gentoo",
                                         sizeof("Gentoo") - 1, nullptr) != 0,
  ...
  do {
    ...
    for (uint32_t i = 0; i < NUM_GROUP_TOX; ++i) {
      ...
      uint32_t peer_count =
          tox_conference_peer_count(autotoxes[i].tox, 0, &err);
