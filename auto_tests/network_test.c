...
START_TEST(test_addr_resolv_localhost)
{
    ...
    ck_assert_msg(!memcmp(&ip.ip4, &IP4_LOOPBACK, sizeof(IP4)), "Expected 127.0.0.1, got %s.",
    ...
    ck_assert_msg(!memcmp(&extra.ip4, &IP4_LOOPBACK, sizeof(IP4)), "Expected 127.0.0.1, got %s.",
                  ip_ntoa(&ip, ip_str, sizeof(ip_str)));
}
END_TEST
