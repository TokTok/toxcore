...
void toxav_iterate(ToxAV *av, void *userdata)
{
    ...
    ToxAVCall *call = av->calls[av->calls_head];

    for (; call; call = call->next) {
        if (call->active) {
            pthread_mutex_lock(call->mutex);
            pthread_mutex_unlock(av->mutex);

            ac_iterate(call->audio.second, userdata);
            vc_iterate(call->video.second, userdata);

            if (call->msi_call->self_capabilities & msi_CapRAudio &&
                    call->msi_call->peer_capabilities & msi_CapSAudio) {
                rc = MIN(call->audio.second->lp_frame_duration, rc);
            }

            if (call->msi_call->self_capabilities & msi_CapRVideo &&
                    call->msi_call->peer_capabilities & msi_CapSVideo) {
                rc = MIN(call->video.second->lcfd, (uint32_t) rc);
            }

            uint32_t fid = call->friend_number;

            pthread_mutex_unlock(call->mutex);
            pthread_mutex_lock(av->mutex);

            /* In case this call is popped from container stop iteration */
            if (call_get(av, fid) != call) {
                break;
            }
        }
    }
...
void callback_bwc(BWController *bwc, uint32_t friend_number, float loss, void *call_data, void *userdata)
{
    ...
    ToxAVCall *call = (ToxAVCall *)call_data;
    assert(call);
