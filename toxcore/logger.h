...
typedef void logger_cb(uint32_t id, LOGGER_LEVEL level, const char *file, int line, const char *func,
                       const char *message, void *userdata);

/**
 * Creates a new logger with logging disabled (callback is NULL) by default.
 */
Logger *logger_new(uint32_t id);
 ...
 */
void logger_callback_log(Logger *log, logger_cb *function);
