...
static void fetch_broadcast_info(uint16_t port) { broadcast_count = 0; }
...
int LAN_ip(IP ip)
{
    ...
    if (ip.family == AF_INET) {
        ...
        if (ip4.uint8[0] == 169 && ip4.uint8[1] == 254 && ip4.uint8[2] != 0 && ip4.uint8[2] != 255) {
            return 0;
        }
...
void LANdiscovery_kill(DHT *dht) { networking_registerhandler(dht->net, NET_PACKET_LAN_DISCOVERY, NULL, NULL); }
