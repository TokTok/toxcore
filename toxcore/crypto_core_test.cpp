...
enum {
    ...
    /**
     * The margin of error (in cycles) we allow for this test.
     *
     * Should be within 0.5% of ~2000 CPU cycles. In reality, the code is much
     * more precise and is usually within 1 CPU cycle.
     */
...
TEST(CryptoCore, MemcmpTimingIsDataIndependent)
{
    ...
    EXPECT_LT(delta, CRYPTO_TEST_MEMCMP_EPS)
            ...
            << "Time of the same data comparation: " << same_median << " cycles\n"
            << "Time of the different data comparation: " << not_same_median << " cycles";
}
