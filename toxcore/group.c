                                 ...
                                 uint16_t length)
{
    ...
    if (member) {
        add_conn_to_groupchat(g_c, friendcon_id, groupnumber, GROUPCHAT_CLOSE_REASON_INTRODUCING, 0);
    }
    ...
    switch (data[0]) {
        case INVITE_ID: {
            ...
            if (groupnumber == -1) {
                ...
                if (g && g->status == GROUPCHAT_STATUS_CONNECTED && g->type == invite_data[sizeof(uint16_t)]) {
        ...
        case INVITE_MEMBER_ID: {
            ...
            if (member) {
                add_conn_to_groupchat(g_c, friendcon_id, groupnum, GROUPCHAT_CLOSE_REASON_INTRODUCER, 0);
                send_peer_query(g_c, friendcon_id, other_groupnum);
            }
