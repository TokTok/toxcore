...
static int addpeer(Group_Chats *g_c, int groupnumber, const uint8_t *real_pk, const uint8_t *temp_pk,
                   uint16_t peer_number, void *userdata)
{
    Group_c *g = get_group_c(g_c, groupnumber);
...
int add_groupchat(Group_Chats *g_c, uint8_t type)
{
    ...
    int peer_index = addpeer(g_c, groupnumber, g->real_pk, g_c->m->dht->self_public_key, 0, NULL);

    if (peer_index == -1) {
        return -1;
    }

    setnick(g_c, groupnumber, peer_index, g_c->m->name, g_c->m->name_length, NULL);
                                        ...
                                        void *userdata)
{
    ...
    switch (data[0]) {
        ...
        case INVITE_RESPONSE_ID: {
            ...
            addpeer(g_c, groupnum, real_pk, temp_pk, peer_number, userdata);
...
static int handle_send_peers(Group_Chats *g_c, int groupnumber, const uint8_t *data, uint16_t length, void *userdata)
{
    ...
    while ((unsigned int)(length - (d - data)) >= sizeof(uint16_t) + crypto_box_PUBLICKEYBYTES * 2 + 1) {
        ...
        d += sizeof(uint16_t);
        int peer_index = addpeer(g_c, groupnumber, d, d + crypto_box_PUBLICKEYBYTES, peer_num, userdata);
        ...
        setnick(g_c, groupnumber, peer_index, d, name_length, userdata);
        d += name_length;
    }
                                        ...
                                        int close_index, void *userdata)
{
    ...
    switch (message_id) {
        ...
        case GROUP_MESSAGE_NAME_ID: {
            if (setnick(g_c, groupnumber, index, msg_data, msg_data_len, userdata) == -1) {
                return;
            }
        }
        break;
