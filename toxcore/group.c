 ...
 */
static uint8_t groupnumber_not_valid(const Tox *tox, int groupnumber)
{
 ...
 */
static int realloc_groupchats(Tox *tox, uint32_t num)
{
    Group_Chats *g_c = tox->gc;
 ...
 */
static int create_group_chat(Tox *tox)
{
    Group_Chats *g_c = tox->gc;
    ...
    if (realloc_groupchats(tox, g_c->num_chats + 1) == 0) {
        id = g_c->num_chats;
        ++g_c->num_chats;
 ...
 * return 0 on success.
 */
static int wipe_group_chat(Tox *tox, int groupnumber)
{
    Group_Chats *g_c = tox->gc;

    if (groupnumber_not_valid(tox, groupnumber))
        return -1;

    uint32_t i;
    sodium_memzero(&(tox->gc->chats[groupnumber]), sizeof(Group_c));
    ...
    if (g_c->num_chats != i) {
        g_c->num_chats = i;
        realloc_groupchats(tox, g_c->num_chats);
    }

    return 0;
}

static Group_c *get_group_c(const Tox *tox, int groupnumber)
{
    if (groupnumber_not_valid(tox, groupnumber))
        return 0;

    return &tox->gc->chats[groupnumber];
}
 ...
 */
static int get_group_num(const Tox *tox, const uint8_t *identifier)
{
    Group_Chats *g_c = tox->gc;
...
static int add_conn_to_groupchat(Tox *tox, int friendcon_id, int groupnumber, uint8_t closest, uint8_t lock);

static int add_to_closest(Tox *tox, int groupnumber, const uint8_t *real_pk, const uint8_t *temp_pk)
{
    Group_Chats *g_c = tox->gc;

    Group_c *g = get_group_c(tox, groupnumber);
    ...
    if (old) {
        add_to_closest(tox, groupnumber, old_real_pk, old_temp_pk);
    }
...
static int connect_to_closest(Tox *tox, int groupnumber)
{
    Group_Chats *g_c = tox->gc;
    ...
    if (g->changed == GROUPCHAT_CLOSEST_REMOVED) {
        for (i = 0; i < g->numpeers; ++i) {
            add_to_closest(tox, groupnumber, g->group[i].real_pk, g->group[i].temp_pk);
        }
    }
    ...
    for (i = 0; i < DESIRED_CLOSE_CONNECTIONS; ++i) {
        ...
        add_conn_to_groupchat(tox, friendcon_id, groupnumber, 1, lock);
 ...
 * return -1 if error.
 */
static int addpeer(Tox *tox, int groupnumber, const uint8_t *real_pk, const uint8_t *temp_pk,
                   uint16_t peer_number)
{
    ...
    add_to_closest(tox, groupnumber, real_pk, temp_pk);

    if (g_c->peer_namelistchange)
        g_c->peer_namelistchange(tox, groupnumber, g->numpeers - 1, CHAT_CHANGE_PEER_ADD,
...
static int remove_close_conn(Tox *tox, int groupnumber, int friendcon_id)
{
    Group_Chats *g_c = tox->gc;
 ...
 * return -1 if error.
 */
static int delpeer(Tox *tox, int groupnumber, int peer_index)
{
    Group_Chats *g_c = tox->gc;
    ...
    if (friendcon_id != -1) {
        remove_close_conn(tox, groupnumber, friendcon_id);
    }
    ...
    if (g_c->peer_namelistchange)
        g_c->peer_namelistchange(tox, groupnumber, peer_index, CHAT_CHANGE_PEER_DEL, g_c->group_namelistchange_userdata);
...
static int setnick(Tox *tox, int groupnumber, int peer_index, const uint8_t *nick, uint16_t nick_len)
{
    Group_Chats *g_c = tox->gc;
    ...
    if (g_c->peer_namelistchange)
        g_c->peer_namelistchange(tox, groupnumber, peer_index, CHAT_CHANGE_PEER_NAME, g_c->group_namelistchange_userdata);

    return 0;
}

static int settitle(Tox *tox, int groupnumber, int peer_index, const uint8_t *title, uint8_t title_len)
{
    Group_Chats *g_c = tox->gc;
    ...
    if (g_c->title_callback)
        g_c->title_callback(tox, groupnumber, peer_index, title, title_len, g_c->title_callback_userdata);

    return 0;
}

static void set_conns_type_close(Tox *tox, int groupnumber, int friendcon_id, uint8_t type)
{
    Group_Chats *g_c = tox->gc;
...
static void set_conns_status_groups(Tox *tox, int friendcon_id, uint8_t type)
{
    Group_Chats *g_c = tox->gc;
    ...
    for (i = 0; i < g_c->num_chats; ++i) {
        set_conns_type_close(tox, i, friendcon_id, type);
    }
}
...
static int handle_status(void *object, int friendcon_id, uint8_t status)
{
    ...
    if (status) { /* Went online */
        set_conns_status_groups(tox, friendcon_id, GROUPCHAT_CLOSE_ONLINE);
    } else { /* Went offline */
        set_conns_status_groups(tox, friendcon_id, GROUPCHAT_CLOSE_CONNECTION);
...
static int add_conn_to_groupchat(Tox *tox, int friendcon_id, int groupnumber, uint8_t closest, uint8_t lock)
{
    ...
    if (lock)
        toxconn_inc_connection_lock(g_c->fr_c, friendcon_id);
...
int add_groupchat(Tox *tox, uint8_t type)
{
    ...
    int groupnumber = create_group_chat(tox);
    ...
    int peer_index = addpeer(tox, groupnumber, g->real_pk, g_c->tox->dht->self_public_key, 0);

    if (peer_index == -1) {
        return -1;
    }

    setnick(tox, groupnumber, peer_index, g_c->m->name, g_c->m->name_length);

    return groupnumber;
}

static int group_kill_peer_send(const Tox *tox, int groupnumber, uint16_t peer_num);
...
int del_groupchat(Tox *tox, int groupnumber)
{
    ...
    group_kill_peer_send(tox, groupnumber, g->peer_number);
    ...
    return wipe_group_chat(tox, groupnumber);
}
                ...
                uint16_t length)
{
    ...
    for (i = 0; i < g->numpeers && i < length; ++i) {
        lengths[i] = group_peername(tox, groupnumber, i, names[i]);
    }
...
int invite_friend(Tox *tox, int32_t friendnumber, int groupnumber)
{
    ...
    if (send_group_invite_packet(g_c->tox, friendnumber, invite, sizeof(invite))) {
        ...
        wipe_group_chat(tox, groupnumber);
        return -1;
    }
}

static unsigned int send_peer_query(Tox *tox, int friendcon_id, uint16_t group_num);
...
int join_groupchat(Tox *tox, int32_t friendnumber, uint8_t expected_type, const uint8_t *data, uint16_t length)
{
    ...
    if (get_group_num(tox, data + sizeof(uint16_t)) != -1)
        return -1;
    ...
    if (send_group_invite_packet(g_c->tox, friendnumber, response, sizeof(response))) {
        ...
        int close_index = add_conn_to_groupchat(tox, friendcon_id, groupnumber, 0, 1);
        ...
        send_peer_query(tox, friendcon_id, other_groupnum);
        return groupnumber;
    } else {
...
static unsigned int send_message_group(const Tox *tox, int groupnumber, uint8_t message_id, const uint8_t *data,
                                       uint16_t len);

#define GROUP_MESSAGE_PING_ID 0
int group_ping_send(const Tox *tox, int groupnumber)
{
    Group_Chats *g_c = tox->gc;

    if (send_message_group(tox, groupnumber, GROUP_MESSAGE_PING_ID, 0, 0)) {
        return 0;
    } else {
        return -1;
    }
 ...
 * return -1 on failure
 */
int group_new_peer_send(const Tox *tox, int groupnumber, uint16_t peer_num, const uint8_t *real_pk,
                        uint8_t *temp_pk)
{
    Group_Chats *g_c = tox->gc;
    ...
    if (send_message_group(tox, groupnumber, GROUP_MESSAGE_NEW_PEER_ID, packet, sizeof(packet))) {
        return 0;
    } else {
        return -1;
    }
...
static int group_kill_peer_send(const Tox *tox, int groupnumber, uint16_t peer_num)
{
    ...
    if (send_message_group(tox, groupnumber, GROUP_MESSAGE_KILL_PEER_ID, packet, sizeof(packet))) {
        return 0;
    } else {
        return -1;
    }
 ...
 * return -1 on failure
 */
static int group_name_send(const Tox *tox, int groupnumber, const uint8_t *nick, uint16_t nick_len)
{
    Group_Chats *g_c = tox->gc;
    ...
    if (send_message_group(tox, groupnumber, GROUP_MESSAGE_NAME_ID, nick, nick_len)) {
        return 0;
    } else {
        return -1;
    }
...
int group_title_send(const Tox *tox, int groupnumber, const uint8_t *title, uint8_t title_len)
{
    ...
    if (send_message_group(tox, groupnumber, GROUP_MESSAGE_TITLE_ID, title, title_len))
        return 0;
    else
        return -1;
}
...
static void handle_friend_invite_packet(Tox *tox, uint32_t friendnumber, const uint8_t *data, uint16_t length)
{
    ...
    switch (data[0]) {
        case INVITE_ID: {
            ...
            int groupnumber = get_group_num(tox, data + 1 + sizeof(uint16_t));
        ...
        case INVITE_RESPONSE_ID: {
            ...
            Group_c *g = get_group_c(tox, groupnum);
            ...
            addpeer(tox, groupnum, real_pk, temp_pk, peer_number);
            int close_index = add_conn_to_groupchat(tox, friendcon_id, groupnum, 0, 1);
            ...
            group_new_peer_send(tox, groupnum, peer_number, real_pk, temp_pk);
            break;
        }
...
static unsigned int send_peer_kill(Tox *tox, int friendcon_id, uint16_t group_num);

static int handle_packet_online(Tox *tox, int friendcon_id, uint8_t *data, uint16_t length)
{
    Group_Chats *g_c = tox->gc;
    ...
    int groupnumber = get_group_num(tox, data + sizeof(uint16_t));
    uint16_t other_groupnum;
    ...
    if (g->number_joined != -1 && count_close_connected(g) >= DESIRED_CLOSE_CONNECTIONS) {
        ...
        if (!g->close[fr_close_index].closest) {
            ...
            send_peer_kill(tox, g->close[fr_close_index].number, g->close[fr_close_index].group_number);
 ...
 * return 0 on failure.
 */
static unsigned int send_peers(Tox *tox, int groupnumber, int friendcon_id, uint16_t group_num)
{
    Group_Chats *g_c = tox->gc;
...
static int handle_send_peers(Tox *tox, int groupnumber, const uint8_t *data, uint16_t length)
{
    Group_Chats *g_c = tox->gc;
    ...
    while ((unsigned int)(length - (d - data)) >= sizeof(uint16_t) + crypto_box_PUBLICKEYBYTES * 2 + 1) {
        ...
        d += sizeof(uint16_t);
        int peer_index = addpeer(tox, groupnumber, d, d + crypto_box_PUBLICKEYBYTES, peer_num);
        ...
        if (g->status == GROUPCHAT_STATUS_VALID
            ...
            g->status = GROUPCHAT_STATUS_CONNECTED;
            group_name_send(tox, groupnumber, g_c->m->name, g_c->m->name_length);
        }
        ...
        setnick(tox, groupnumber, peer_index, d, name_length);
        d += name_length;
    }
...
static void handle_direct_packet(Tox *tox, int groupnumber, const uint8_t *data, uint16_t length,
                                 int close_index)
{
    Group_Chats *g_c = tox->gc;
    ...
    switch (data[0]) {
        ...
        case PEER_QUERY_ID: {
            ...
            send_peers(tox, groupnumber, g->close[close_index].number, g->close[close_index].group_number);
        }

        break;

        case PEER_RESPONSE_ID: {
            handle_send_peers(tox, groupnumber, data + 1, length - 1);
        }

        break;

        case PEER_TITLE_ID: {
            settitle(tox, groupnumber, -1, data + 1, length - 1);
        }
 ...
 */
static unsigned int send_message_all_close(const Tox *tox, int groupnumber, const uint8_t *data,
 ...
 * return number of messages sent.
 */
static unsigned int send_lossy_all_close(const Tox *tox, int groupnumber, const uint8_t *data, uint16_t length,
        int receiver)
{
    Group_Chats *g_c = tox->gc;
                                       ...
                                       uint16_t len)
{
    ...
    return send_message_all_close(tox, groupnumber, packet, sizeof(packet), -1);
}
...
int group_message_send(const Tox *tox, int groupnumber, const uint8_t *message, uint16_t length)
{
    ...
    if (send_message_group(tox, groupnumber, PACKET_ID_MESSAGE, message, length)) {
        return 0;
    } else {
        return -1;
    }
...
int group_action_send(const Tox *tox, int groupnumber, const uint8_t *action, uint16_t length)
{
    ...
    if (send_message_group(tox, groupnumber, PACKET_ID_ACTION, action, length)) {
        return 0;
    } else {
        return -1;
    }
...
int send_group_lossy_packet(const Tox *tox, int groupnumber, const uint8_t *data, uint16_t length)
{
    ...
    if (send_lossy_all_close(tox, groupnumber, packet, sizeof(packet), -1) == 0) {
        return -1;
    }
...
static void handle_message_packet_group(Tox *tox, int groupnumber, const uint8_t *data, uint16_t length,
                                        int close_index)
{
    Group_Chats *g_c = tox->gc;
    ...
    if (index == -1) {
        ...
        send_peer_query(tox, g->close[close_index].number, g->close[close_index].group_number);
        return;
    }
    ...
    switch (message_id) {
        ...
        case GROUP_MESSAGE_NEW_PEER_ID: {
            ...
            addpeer(tox, groupnumber, msg_data + sizeof(uint16_t), msg_data + sizeof(uint16_t) + crypto_box_PUBLICKEYBYTES,
                    new_peer_number);
        }
        break;
        ...
        case GROUP_MESSAGE_NAME_ID: {
            if (setnick(tox, groupnumber, index, msg_data, msg_data_len) == -1)
                return;
        }
        break;

        case GROUP_MESSAGE_TITLE_ID: {
            if (settitle(tox, groupnumber, index, msg_data, msg_data_len) == -1)
                return;
        }
        break;
        ...
        case PACKET_ID_MESSAGE: {
            ...
            if (g_c->message_callback)
                g_c->message_callback(tox, groupnumber, index, newmsg, msg_data_len, g_c->message_callback_userdata);
        ...
        case PACKET_ID_ACTION: {
            ...
            if (g_c->action_callback)
                g_c->action_callback(tox, groupnumber, index, newmsg, msg_data_len, g_c->action_callback_userdata);
    ...
    send_message_all_close(tox, groupnumber, data, length, -1/*TODO close_index*/);
}
...
static int handle_packet(void *object, int friendcon_id, uint8_t *data, uint16_t length)
{
    ...
    if (data[0] == PACKET_ID_ONLINE_PACKET) {
        return handle_packet_online(tox, friendcon_id, data + 1, length - 1);
    }
    ...
    switch (data[0]) {
        case PACKET_ID_DIRECT_GROUPCHAT: {
            handle_direct_packet(tox, groupnumber, data + 1 + sizeof(uint16_t), length - (1 + sizeof(uint16_t)), index);
            break;
        }

        case PACKET_ID_MESSAGE_GROUPCHAT: {
            handle_message_packet_group(tox, groupnumber, data + 1 + sizeof(uint16_t), length - (1 + sizeof(uint16_t)), index);
            break;
        }
...
static int handle_lossy(void *object, int friendcon_id, const uint8_t *data, uint16_t length)
{
    ...
    send_lossy_all_close(tox, groupnumber, data + 1 + sizeof(uint16_t), length - (1 + sizeof(uint16_t)), index);
    return 0;
}
...
static int ping_groupchat(Tox *tox, int groupnumber)
{
    Group_Chats *g_c = tox->gc;
    ...
    if (is_timeout(g->last_sent_ping, GROUP_PING_INTERVAL)) {
        if (group_ping_send(tox, groupnumber) != -1) /* Ping */
...
static int groupchat_clear_timedout(Tox *tox, int groupnumber)
{
    Group_Chats *g_c = tox->gc;
...
void send_name_all_groups(Tox *tox)
{
    ...
    for (i = 0; i < g_c->num_chats; ++i) {
        Group_c *g = get_group_c(tox, i);

        if (!g)
            continue;

        if (g->status == GROUPCHAT_STATUS_CONNECTED) {
            group_name_send(tox, i, g_c->m->name, g_c->m->name_length);
        }
    }
}
...
void do_groupchats(Tox *tox)
{
    ...
    for (i = 0; i < g_c->num_chats; ++i) {
        ...
        if (g->status == GROUPCHAT_STATUS_CONNECTED) {
            connect_to_closest(tox, i);
            ping_groupchat(tox, i);
            groupchat_clear_timedout(tox, i);
        }
    }
