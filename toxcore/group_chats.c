...
on_error:
    gc_peer_delete(m, groupnumber, peernumber, (const uint8_t *)"BAD SSTATE", 10);
                                      ...
                                      bool direct_conn)
{
    ...
    /* Duplicate packet */
    if (lossless_ret == 0) {
        fprintf(stderr, "got duplicate packet %llu (type %u)\n", message_id, packet_type);
        return gc_send_message_ack(chat, peernumber, message_id, 0);
    }

    /* request missing packet */
    if (lossless_ret == 1) {
        fprintf(stderr, "recieved out of order packet. expeceted %llu, got %llu\n", gconn->recv_message_id + 1, message_id);
...
static int init_gc_tcp_connection(Messenger *m, GC_Chat *chat)
{
    ...
    uint16_t num_relays = tcp_connections_count(m->net_crypto->tcp_c);
...
int gc_invite_friend(GC_Session *c, GC_Chat *chat, int32_t friendnumber,
                     int send_invite_packet(const Messenger *m, uint32_t friendnumber, const uint8_t *packet, size_t length))
{
    uint8_t packet[MAX_GC_PACKET_SIZE];
    ...
    if (send_invite_packet(c->messenger, friendnumber, packet, length) == -1) {
        return -2;
    }
