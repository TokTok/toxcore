...
#define GC_MIN_PLAIN_HS_PACKET_SIZE                                            \
  (sizeof(uint8_t) + HASH_ID_BYTES + ENC_PUBLIC_KEY + SIG_PUBLIC_KEY +         \
   sizeof(uint8_t) + sizeof(uint8_t) + sizeof(uint32_t))

/* The minimum size of an encrypted group handshake packet */
#define GC_MIN_ENCRYPTED_HS_PACKET_SIZE                                        \
  (sizeof(uint8_t) + HASH_ID_BYTES + ENC_PUBLIC_KEY + CRYPTO_NONCE_SIZE +      \
   GC_MIN_PLAIN_HS_PACKET_SIZE + CRYPTO_MAC_SIZE)

/* Size of a group's shared state in packed format */
#define GC_PACKED_SHARED_STATE_SIZE                                            \
  (EXT_PUBLIC_KEY + sizeof(uint32_t) + MAX_GC_GROUP_NAME_SIZE +                \
   sizeof(uint16_t) + sizeof(uint8_t) + sizeof(uint16_t) +                     \
   MAX_GC_PASSWORD_SIZE + GC_MODERATION_HASH_SIZE + sizeof(uint32_t))

/* Minimum size of a topic packet; includes topic length, public signature key,
 * and the topic version */
#define GC_MIN_PACKED_TOPIC_INFO_SIZE                                          \
  (sizeof(uint16_t) + SIG_PUBLIC_KEY + sizeof(uint32_t))

#define GC_SHARED_STATE_ENC_PACKET_SIZE                                        \
  (HASH_ID_BYTES + SIGNATURE_SIZE + GC_PACKED_SHARED_STATE_SIZE)

/* Header information attached to all broadcast messages. broadcast_type, public
 * key hash, timestamp */
...
#define GC_MIN_LOSSLESS_PACKET_SIZE                                            \
  (sizeof(uint8_t) + GC_MESSAGE_ID_BYTES + HASH_ID_BYTES + ENC_PUBLIC_KEY +    \
   CRYPTO_NONCE_SIZE + sizeof(uint8_t) + CRYPTO_MAC_SIZE)

/* Smallest possible size of a lossy group packet */
#define GC_MIN_LOSSY_PACKET_SIZE                                               \
  (GC_MIN_LOSSLESS_PACKET_SIZE - GC_MESSAGE_ID_BYTES)

/* Maximum size of a group packet */
#define GC_MAX_PACKET_SIZE (65507)

/* Approximation of the sync response packet size limit */
#define GC_MAX_NUM_PEERS                                                       \
  (GC_MAX_PACKET_SIZE / (ENC_PUBLIC_KEY + sizeof(IP_Port)))

/* Maximum number of bytes to pad packets with */
#define GC_MAX_PACKET_PADDING (8)

/* Size of a ping packet, which contains the peer count, shared state version,
 * sanctions list version and topic version */
...
static int peer_add(Messenger *m, int group_number, IP_Port *ipp,
                    const uint8_t *public_key);
static int peer_update(Messenger *m, int group_number, GC_GroupPeer *peer,
                       uint32_t peer_number);
static int group_delete(GC_Session *c, GC_Chat *chat);
static void group_cleanup(GC_Session *c, GC_Chat *chat);
static int get_nick_peer_number(const GC_Chat *chat, const uint8_t *nick,
                                uint16_t length);
...
void pack_group_info(const GC_Chat *chat, Saved_Group *temp,
                     bool can_use_cached_value) {
  // copy info from cached save struct if possible (for disconnected groups
  // only)
  ...
  memcpy(temp->founder_public_key, chat->shared_state.founder_public_key,
         EXT_PUBLIC_KEY);
  temp->group_name_length = net_htons(chat->shared_state.group_name_len);
  memcpy(temp->group_name, chat->shared_state.group_name,
         MAX_GC_GROUP_NAME_SIZE);
  ...
  memcpy(temp->mod_list_hash, chat->shared_state.mod_list_hash,
         GC_MODERATION_HASH_SIZE);
  ...
  memcpy(temp->topic_public_sig_key, chat->topic_info.public_sig_key,
         SIG_PUBLIC_KEY);
  ...
  memcpy(temp->chat_secret_key, chat->chat_secret_key,
         EXT_SECRET_KEY); /* empty for non-founders */

  uint16_t num_addrs =
      gc_copy_peer_addrs(chat, temp->addrs, GROUP_SAVE_MAX_PEERS);
  ...
  temp->group_connection_state =
      is_manually_disconnected ? SGCS_DISCONNECTED : SGCS_CONNECTED;
...
static bool is_peer_confirmed(const GC_Chat *chat, const uint8_t *peer_pk) {
  ...
  for (i = 0; i < MAX_GC_CONFIRMED_PEERS; ++i) {
    if (!memcmp(chat->confirmed_peers[i], peer_pk,
                ENC_PUBLIC_KEY)) { // peer in our list
      return true;
    }
  }
 ...
 */
static int get_peernum_of_enc_pk(const GC_Chat *chat,
                                 const uint8_t *public_enc_key) {
  uint32_t i;

  for (i = 0; i < chat->numpeers; ++i) {
    if (memcmp(chat->gcc[i].addr.public_key, public_enc_key, ENC_PUBLIC_KEY) ==
        0) {
      return i;
    }
  }
 ...
 * return -1 if peer is not in chat.
 */
static int get_peernum_of_sig_pk(const GC_Chat *chat,
                                 const uint8_t *public_sig_key) {
  uint32_t i;

  for (i = 0; i < chat->numpeers; ++i) {
    if (memcmp(get_sig_pk(chat->gcc[i].addr.public_key), public_sig_key,
               SIG_PUBLIC_KEY) == 0) {
      return i;
    }
  }
...
static int validate_gc_peer_role(const GC_Chat *chat, uint32_t peer_number) {
  ...
  case GR_MODERATOR: {
    if (mod_list_index_of_sig_pk(chat, get_sig_pk(gconn->addr.public_key)) ==
        -1) {
      return -1;
    }
  ...
  case GR_OBSERVER: {
    /* Don't validate self as this is called when we don't have the sanctions
     * list yet */
    if (!sanctions_list_is_observer(chat, gconn->addr.public_key) &&
        peer_number != 0) {
      return -1;
 ...
 *
 * These ID's are permanently assigned to a peer when they join the group and
 * should be considered arbitrary values. TODO: This could probably be done a
 * better way.
 */
 ...
 */
static int set_gc_password_local(GC_Chat *chat, const uint8_t *passwd,
                                 uint16_t length) {
...
/* Expands the chat_id into the extended chat public key (encryption key +
 * signature key) dest must have room for EXT_PUBLIC_KEY bytes.
 */
 ...
 */
uint16_t gc_copy_peer_addrs(const GC_Chat *chat, GC_SavedPeerInfo *addrs,
                            size_t max_addrs) {
  uint32_t i;
  uint16_t num = 0;
...
static int update_gc_sanctions_list(GC_Chat *chat,
                                    const uint8_t *public_sig_key);
 ...
 *
 * TODO: Make this smarter in who to remove (e.g. the mod who hasn't been seen
 * online in the longest time)
 *
 * Returns 0 on success.
...
/* Size of peer data that we pack for transfer (nick length must be accounted
 * for separately). packed data includes: nick, nick length, status, role
 */
#define PACKED_GC_PEER_SIZE                                                    \
  (MAX_GC_NICK_SIZE + sizeof(uint16_t) + sizeof(uint8_t) + sizeof(uint8_t))
 ...
 * Return -1 on failure.
 */
static int pack_gc_peer(uint8_t *data, uint16_t length,
                        const GC_GroupPeer *peer) {
 ...
 */
static int unpack_gc_peer(GC_GroupPeer *peer, const uint8_t *data,
                          uint16_t length) {
  if (PACKED_GC_PEER_SIZE > length) {
    return -1;
  }
...
/* Packs shared_state into data. data must have room for at least
 * GC_PACKED_SHARED_STATE_SIZE bytes.
 *
 * Returns packed data length.
 */
static uint16_t pack_gc_shared_state(uint8_t *data, uint16_t length,
                                     const GC_SharedState *shared_state) {
  ...
  memcpy(data + packed_len, shared_state->mod_list_hash,
         GC_MODERATION_HASH_SIZE);
...
/* Unpacks shared state data into shared_state. data must contain at least
 * GC_PACKED_SHARED_STATE_SIZE bytes.
 *
 * Returns the length of processed data.
 */
static uint16_t unpack_gc_shared_state(GC_SharedState *shared_state,
  ...
  memcpy(shared_state->founder_public_key, data + len_processed,
         EXT_PUBLIC_KEY);
  ...
  shared_state->group_name_len =
      min_u16(shared_state->group_name_len, MAX_GC_GROUP_NAME_SIZE);
  len_processed += sizeof(uint16_t);
  memcpy(shared_state->group_name, data + len_processed,
         MAX_GC_GROUP_NAME_SIZE);
  ...
  memcpy(shared_state->mod_list_hash, data + len_processed,
         GC_MODERATION_HASH_SIZE);
 ...
 * Returns packed data length.
 */
static uint16_t pack_gc_topic_info(uint8_t *data, uint16_t length,
                                   const GC_TopicInfo *topic_info) {
 ...
 */
static int unpack_gc_topic_info(GC_TopicInfo *topic_info, const uint8_t *data,
                                uint16_t length) {
  ...
  if (length - sizeof(uint16_t) <
      topic_info->length + SIG_PUBLIC_KEY + sizeof(uint32_t)) {
    return -1;
  }
 ...
 * Packet includes self pk hash, shared state signature, and packed shared state
 * info. data must have room for at least GC_SHARED_STATE_ENC_PACKET_SIZE bytes.
 *
...
static int make_gc_shared_state_packet(const GC_Chat *chat, uint8_t *data,
  ...
  uint16_t packed_len = pack_gc_shared_state(
      data + HASH_ID_BYTES + SIGNATURE_SIZE,
      length - HASH_ID_BYTES - SIGNATURE_SIZE, &chat->shared_state);
...
/* Creates a signature for the group's shared state in packed form and
 * increments the version. This should only be called by the founder.
 *
 * Returns 0 on success.
 * Returns -1 on failure.
 */
static int sign_gc_shared_state(GC_Chat *chat) {
  ...
  if (chat->shared_state.version !=
      UINT32_MAX) { /* improbable, but an overflow would break everything */
    ++chat->shared_state.version;
  }

  uint8_t shared_state[GC_PACKED_SHARED_STATE_SIZE];
  uint16_t packed_len = pack_gc_shared_state(shared_state, sizeof(shared_state),
                                             &chat->shared_state);
  ...
  int ret = crypto_sign_detached(chat->shared_state_sig, nullptr, shared_state,
                                 packed_len, get_sig_sk(chat->chat_secret_key));
 ...
 * Returns -1 on failure.
 */
static int unwrap_group_packet(const uint8_t *shared_key, uint8_t *data,
                               uint64_t *message_id, uint8_t *packet_type,
                               const uint8_t *packet, uint16_t length) {
  uint8_t plain[GC_MAX_PACKET_SIZE];
  uint8_t nonce[CRYPTO_NONCE_SIZE];
  memcpy(nonce, packet + sizeof(uint8_t) + HASH_ID_BYTES + ENC_PUBLIC_KEY,
         CRYPTO_NONCE_SIZE);

  int plain_len =
      decrypt_data_symmetric(shared_key, nonce,
                             packet + sizeof(uint8_t) + HASH_ID_BYTES +
                                 ENC_PUBLIC_KEY + CRYPTO_NONCE_SIZE,
                             length - (sizeof(uint8_t) + HASH_ID_BYTES +
                                       ENC_PUBLIC_KEY + CRYPTO_NONCE_SIZE),
                             plain);
 ...
 *
 * Adds encrypted header consisting of: packet type, message_id (only for
 * lossless packets) Adds plaintext header consisting of: packet identifier,
 * chat_id_hash, self public encryption key, nonce.
 *
 * Returns length of encrypted packet on success.
 * Returns -1 on failure.
 */
static int wrap_group_packet(const uint8_t *self_pk, const uint8_t *shared_key,
                             uint8_t *packet, uint32_t packet_size,
                             const uint8_t *data, uint32_t length,
                             uint64_t message_id, uint8_t packet_type,
                             uint32_t chat_id_hash, uint8_t packet_id) {
  uint16_t padding_len = gc_packet_padding_length(length);

  if (length + padding_len + CRYPTO_MAC_SIZE + 1 + HASH_ID_BYTES +
          ENC_PUBLIC_KEY + CRYPTO_NONCE_SIZE >
      packet_size) {
    return -1;
  }
  ...
  int enc_len =
      encrypt_data_symmetric(shared_key, nonce, plain, plain_len, encrypt);
  ...
  memcpy(packet + sizeof(uint8_t) + HASH_ID_BYTES + ENC_PUBLIC_KEY, nonce,
         CRYPTO_NONCE_SIZE);
  memcpy(packet + sizeof(uint8_t) + HASH_ID_BYTES + ENC_PUBLIC_KEY +
             CRYPTO_NONCE_SIZE,
         encrypt, enc_len);
 ...
 * Returns -1 on failure.
 */
static int send_lossy_group_packet(const GC_Chat *chat, GC_Connection *gconn,
                                   const uint8_t *data, uint32_t length,
                                   uint8_t packet_type) {
  if (!gconn->handshaked) {
    return -1;
  ...
  int len = wrap_group_packet(chat->self_public_key, gconn->shared_key, packet,
                              sizeof(packet), data, length, 0, packet_type,
                              chat->chat_id_hash, NET_PACKET_GC_LOSSY);
 ...
 * Returns -1 on failure.
 */
static int send_lossless_group_packet(GC_Chat *chat, GC_Connection *gconn,
                                      const uint8_t *data, uint32_t length,
                                      uint8_t packet_type) {
  if (!gconn->handshaked) {
    return -1;
  ...
  uint8_t packet[GC_MAX_PACKET_SIZE];
  int len =
      wrap_group_packet(chat->self_public_key, gconn->shared_key, packet,
                        sizeof(packet), data, length, message_id, packet_type,
                        chat->chat_id_hash, NET_PACKET_GC_LOSSLESS);
  ...
  if (gcc_add_to_send_array(chat->mono_time, gconn, packet, len, packet_type) ==
      -1) {
    return -1;
  }
...
static int send_gc_sync_request(GC_Chat *chat, GC_Connection *gconn,
  ...
  memcpy(data + HASH_ID_BYTES + sizeof(uint32_t), chat->shared_state.password,
         MAX_GC_PASSWORD_SIZE);

  return send_lossless_group_packet(chat, gconn, data, length, GP_SYNC_REQUEST);
}

static int send_gc_sync_response(GC_Chat *chat, GC_Connection *gconn,
                                 const uint8_t *data, uint32_t length) {
  return send_lossless_group_packet(chat, gconn, data, length,
                                    GP_SYNC_RESPONSE);
}

static int send_new_peer_announcement(GC_Chat *chat, GC_Connection *gconn,
                                      const uint8_t *data, uint32_t length) {
  return send_lossless_group_packet(chat, gconn, data, length,
                                    GP_PEER_ANNOUNCE);
}

static int send_gc_peer_exchange(const GC_Session *c, GC_Chat *chat,
                                 GC_Connection *gconn);

static int send_gc_handshake_packet(GC_Chat *chat, uint32_t peer_number,
                                    uint8_t handshake_type,
                                    uint8_t request_type, uint8_t join_type);

static int send_gc_oob_handshake_packet(GC_Chat *chat, uint32_t peer_number,
                                        uint8_t handshake_type,
                                        uint8_t request_type,
                                        uint8_t join_type);

static int handle_gc_sync_response(Messenger *m, int group_number,
                                   int peer_number, GC_Connection *gconn,
                                   const uint8_t *data, uint32_t length) {
  ...
  if (num_peers) {
    ...
    if (peers_length / GCA_ANNOUNCE_MIN_SIZE <
        num_peers) { // num peers is invalid
      return -1;
    }

    GC_Announce *announces =
        (GC_Announce *)malloc(sizeof(GC_Announce) * num_peers);
    ...
    int unpacked_announces = gca_unpack_announces_list(
        announces_pointer, peers_length, announces, num_peers, nullptr);
    ...
    for (i = 0; i < num_peers; ++i) {
      ...
      if (!memcmp(curr_announce->peer_public_key, chat->self_public_key,
                  ENC_PUBLIC_KEY)) { // our own info
        continue;
      }
      ...
      IP_Port *ip_port =
          curr_announce->ip_port_is_set ? &curr_announce->ip_port : nullptr;
      int new_peer_number = peer_add(c->messenger, group_number, ip_port,
      ...
      if (curr_announce->ip_port_is_set && !curr_announce->tcp_relays_count) {
        send_gc_handshake_packet(chat, (uint32_t)new_peer_number, GH_REQUEST,
        ...
        const uint64_t timeout =
            mono_time_get(chat->mono_time) + HANDSHAKE_SENDING_TIMEOUT;
...
int gcc_copy_tcp_relay(GC_Connection *gconn, Node_format *node) {
  ...
  int index = (gconn->tcp_relays_index - 1 + MAX_FRIEND_TCP_CONNECTIONS) %
              MAX_FRIEND_TCP_CONNECTIONS;
...
static bool create_announce_for_peer(GC_Chat *chat, GC_Connection *gconn,
                                     uint32_t peer_number,
 ...
 * Additionally sends the group topic, shared state, mod list and sanctions list
 * in respective packets.
 *
 * If the group is password protected the password in the request data must
 * first be verified.
 *
 * Returns non-negative value on success.
 * Returns -1 on failure.
 */
static int handle_gc_sync_request(const Messenger *m, int group_number,
                                  int peer_number, GC_Connection *gconn,
                                  const uint8_t *data, uint32_t length) {
  ...
  if (chat->shared_state.password_length > 0) {
    ...
    if (memcmp(chat->shared_state.password, password,
               chat->shared_state.password_length) != 0) {
  ...
  uint32_t i, num = 0;
  GC_Announce *existing_peers_announces =
      (GC_Announce *)malloc(sizeof(GC_Announce) * (chat->numpeers - 1));
  ...
  if (!create_announce_for_peer(chat, gconn, (uint32_t)peer_number,
                                &new_peer_announce)) {
  ...
  int announce_length = gca_pack_announce(
      sender_relay_data + HASH_ID_BYTES,
      sizeof(sender_relay_data) - HASH_ID_BYTES, &new_peer_announce);
  ...
  for (i = 1; i < chat->numpeers; ++i) {
    if (chat->gcc[i].public_key_hash != gconn->public_key_hash &&
        chat->gcc[i].confirmed && i != peer_number) {
      ...
      if (!create_announce_for_peer(chat, peer_gconn, i,
                                    &existing_peers_announces[num])) {
        continue;
      }

      // sends new peer announce to selected existing peer
      send_new_peer_announcement(chat, peer_gconn, sender_relay_data,
                                 sender_data_length);
      ++num;
    }
  }
  ...
  int announces_count = gca_pack_announces_list(
      response + len, sizeof(response) - len, existing_peers_announces, num,
      &packed_announces_length);
...
static int save_tcp_relay(GC_Connection *gconn, Node_format *node) {
  ...
  memcpy(&gconn->connected_tcp_relays[gconn->tcp_relays_index], node,
         sizeof(Node_format));
  gconn->tcp_relays_index =
      (gconn->tcp_relays_index + 1) % MAX_FRIEND_TCP_CONNECTIONS;
  gconn->any_tcp_connections = true;

  return 0;
}

/* Shares our TCP relays with peer and adds shared relays to our connection with
 * them.
 *
 * Returns 0 on success.
 * Returns -1 on failure.
 */
static int send_gc_tcp_relays(const Mono_Time *mono_time, GC_Chat *chat,
                              GC_Connection *gconn) {
  Node_format tcp_relays[GCC_MAX_TCP_SHARED_RELAYS];
  unsigned int i, num = tcp_copy_connected_relays(chat->tcp_conn, tcp_relays,
                                                  GCC_MAX_TCP_SHARED_RELAYS);
  ...
  for (i = 0; i < num; ++i) {
    add_tcp_relay_connection(chat->tcp_conn, gconn->tcp_connection_num,
                             tcp_relays[i].ip_port, tcp_relays[i].public_key);
  }

  int nodes_len =
      pack_nodes(data + length, sizeof(data) - length, tcp_relays, num);
 ...
 * Returns -1 on failure.
 */
static int handle_gc_tcp_relays(Messenger *m, int group_number,
  ...
  int num_nodes = unpack_nodes(tcp_relays, GCC_MAX_TCP_SHARED_RELAYS, nullptr,
                               data, length, 1);
...
/* Send invite request to peer_number. Invite packet contains your nick and the
 * group password. If no group password is necessary the password field will be
 * ignored by the invitee.
 *
 * Return -1 if fail
 * Return 0 if success
 */
 ...
 * Return 0 if success
 */
static int handle_gc_invite_response(Messenger *m, int group_number,
                                     GC_Connection *gconn, const uint8_t *data,
                                     uint32_t length) {
  GC_Session *c = m->group_handler;
  GC_Chat *chat = gc_get_group(c, group_number);
...
static int handle_gc_invite_response_reject(Messenger *m, int group_number,
                                            const uint8_t *data,
                                            uint32_t length) {
...
static int send_gc_invite_response_reject(GC_Chat *chat, GC_Connection *gconn,
                                          uint8_t type) {
  ...
  return send_lossy_group_packet(chat, gconn, data, length,
                                 GP_INVITE_RESPONSE_REJECT);
}

/* Handles an invite request.
 *
 * Verifies that the invitee's nick is not already taken, and that the correct
 * password has been supplied if the group is password protected.
 *
 * Returns non-negative value on success.
 * Returns -1 on failure.
 */
static int handle_gc_invite_request(Messenger *m, int group_number,
  ...
  if (peer_number_by_nick != -1 &&
      peer_number_by_nick != peer_number) { // in case of duplicate invite
...
static void send_gc_lossless_packet_all_peers(GC_Chat *chat,
                                              const uint8_t *data,
...
static void send_gc_lossy_packet_all_peers(GC_Chat *chat, const uint8_t *data,
                                           uint32_t length, uint8_t type) {
  uint32_t i;
 ...
 */
static uint32_t make_gc_broadcast_header(GC_Chat *chat, const uint8_t *data,
 ...
 * Returns -1 on failure.
 */
static int send_gc_broadcast_message(GC_Chat *chat, const uint8_t *data,
                                     uint32_t length, uint8_t bc_type) {
  ...
  uint32_t packet_len =
      make_gc_broadcast_header(chat, data, length, packet, bc_type);
...
/* Compares a peer's group sync info that we received in a ping packet to our
 * own.
 *
 * If their info appears to be more recent than ours we will first set a sync
 * request flag. If the flag is already set we send a sync request to this peer
 * then set the flag back to false.
 *
 * This function should only be called from handle_gc_ping().
 */
static void do_gc_peer_state_sync(GC_Chat *chat, GC_Connection *gconn,
 ...
 */
static int handle_gc_ping(Messenger *m, int group_number, GC_Connection *gconn,
                          const uint8_t *data, uint32_t length) {
  if (length != GC_PING_PACKET_DATA_SIZE) {
    return -1;
  }
...
static int handle_gc_status(Messenger *m, int group_number,
                            uint32_t peer_number, const uint8_t *data,
                            uint32_t length) {
  if (length != sizeof(uint8_t)) {
    return -1;
  ...
  if (c->status_change) {
    (*c->status_change)(m, group_number, chat->group[peer_number].peer_id,
                        status, c->status_change_userdata);
  }
...
/* Sends self peer info to peer_number. If the group is password protected the
 * request will contain the group password, which the recipient will validate in
 * the respective group message handler.
 *
 * Returns non-negative value on success.
 * Returns -1 on failure.
 */
static int send_self_to_peer(const GC_Session *c, GC_Chat *chat,
                             GC_Connection *gconn) {
  GC_GroupPeer self;
  ...
  uint8_t data[GC_MAX_PACKET_SIZE];
  net_pack_u32(data, chat->self_public_key_hash);
  memcpy(data + HASH_ID_BYTES, chat->shared_state.password,
         MAX_GC_PASSWORD_SIZE);
  ...
  if (packed_len <= 0) {
    fprintf(stderr,
            "pack_gc_peer failed in handle_gc_peer_info_request_request %d\n",
            packed_len);
    return -1;
  }
...
static int handle_gc_peer_info_request(Messenger *m, int group_number,
                                       GC_Connection *gconn) {
  GC_Session *c = m->group_handler;
  GC_Chat *chat = gc_get_group(c, group_number);
  ...
  if (!gconn->confirmed &&
      get_gc_confirmed_numpeers(chat) >= chat->shared_state.maxpeers) {
    return -1;
  }
...
static int handle_gc_peer_announcement(Messenger *m, int group_number,
                                       const uint8_t *data, uint32_t length) {
  ...
  GC_Announce new_peer_announce;
  int unpack_result =
      gca_unpack_announce((const uint8_t *)data, length, &new_peer_announce);
  ...
  IP_Port *ip_port =
      new_peer_announce.ip_port_is_set ? &new_peer_announce.ip_port : nullptr;
  ...
  int peer_number =
      peer_add(m, group_number, ip_port, new_peer_announce.peer_public_key);
...
/* Updates peer's info, validates their group role, and sets them as a confirmed
 * peer. If the group is password protected the password must first be
 * validated.
 *
 * Returns 0 on success.
 * Returns -1 on failure.
 */
static int handle_gc_peer_info_response(Messenger *m, int group_number,
                                        uint32_t peer_number,
                                        const uint8_t *data, uint32_t length) {
  ...
  if (unpack_gc_peer(&peer, data + MAX_GC_PASSWORD_SIZE,
                     length - MAX_GC_PASSWORD_SIZE) == -1) {
...
static int send_peer_shared_state(GC_Chat *chat, GC_Connection *gconn) {
  ...
  return send_lossless_group_packet(chat, gconn, packet, length,
                                    GP_SHARED_STATE);
}
...
/* Compares old_shared_state with the chat instance's current shared state and
 * triggers the appropriate callback depending on what piece of state
 * information changed. Also handles DHT announcement/removal if the privacy
 * state changed.
 *
 * The initial retrieval of the shared state on group join will be ignored by
 * this function.
 */
static void do_gc_shared_state_changes(GC_Session *c, GC_Chat *chat,
                                       const GC_SharedState *old_shared_state) {
  /* Max peers changed */
  if (chat->shared_state.maxpeers != old_shared_state->maxpeers) {
    if (c->peer_limit) {
      (*c->peer_limit)(c->messenger, chat->group_number,
                       chat->shared_state.maxpeers, c->peer_limit_userdata);
    }
  }
  ...
  if (chat->shared_state.privacy_state != old_shared_state->privacy_state) {
    if (c->privacy_state) {
      (*c->privacy_state)(c->messenger, chat->group_number,
  ...
  if (chat->shared_state.password_length != old_shared_state->password_length ||
      memcmp(chat->shared_state.password, old_shared_state->password,
             old_shared_state->password_length) != 0) {

    if (c->password) {
      (*c->password)(c->messenger, chat->group_number,
                     chat->shared_state.password,
...
static int handle_gc_shared_state_error(Messenger *m, int group_number,
  ...
  /* If we don't already have a valid shared state we will automatically try to
     get another invite. Otherwise we attempt to ask a different peer for a
     sync. */
  gc_peer_delete(m, group_number, peer_number,
                 (const uint8_t *)"BAD SHARED STATE", 10, false);
 ...
 * Returns -1 on failure.
 */
static int handle_gc_shared_state(Messenger *m, int group_number,
                                  uint32_t peer_number, const uint8_t *data,
                                  uint32_t length) {
  GC_Session *c = m->group_handler;
  GC_Chat *chat = gc_get_group(c, group_number);
  ...
  if (crypto_sign_verify_detached(signature, ss_data,
                                  GC_PACKED_SHARED_STATE_SIZE,
...
/* Handles new mod_list and compares its hash against the mod_list_hash in the
 * shared state.
 *
 * Returns 0 on success.
 * Returns -1 on failure.
 */
static int handle_gc_mod_list(Messenger *m, int group_number,
                              uint32_t peer_number, const uint8_t *data,
                              uint32_t length) {
  if (length < sizeof(uint16_t)) {
    return -1;
  ...
  if (mod_list_unpack(chat, data + sizeof(uint16_t), length - sizeof(uint16_t),
                      num_mods) == -1) {
    goto ON_ERROR;
  }
  ...
  if (memcmp(mod_list_hash, chat->shared_state.mod_list_hash,
             GC_MODERATION_HASH_SIZE) != 0) {
    goto ON_ERROR;
  }
...
static int handle_gc_sanctions_list_error(Messenger *m, int group_number,
  ...
  gc_peer_delete(m, group_number, peer_number, (const uint8_t *)"BAD SCREDS",
                 10, false);
...
static int handle_gc_sanctions_list(Messenger *m, int group_number,
                                    uint32_t peer_number, const uint8_t *data,
                                    uint32_t length) {
  if (length < sizeof(uint32_t)) {
    return -1;
  ...
  struct GC_Sanction *sanctions =
      (struct GC_Sanction *)malloc(num_sanctions * sizeof(struct GC_Sanction));

  if (sanctions == nullptr) {
    return -1;
  }

  int unpacked_num = sanctions_list_unpack(sanctions, &creds, num_sanctions,
                                           data + sizeof(uint32_t),
                                           length - sizeof(uint32_t), nullptr);

  if (unpacked_num != num_sanctions) {
    fprintf(stderr,
            "sanctions_list_unpack failed in handle_gc_sanctions_list: %d\n",
            unpacked_num);
    free(sanctions);
    return handle_gc_sanctions_list_error(m, group_number, peer_number, chat);
  }

  if (sanctions_list_check_integrity(chat, &creds, sanctions, num_sanctions) ==
      -1) {
    fprintf(
        stderr,
        "sanctions_list_check_integrity failed in handle_gc_sanctions_list\n");
    free(sanctions);
    return handle_gc_sanctions_list_error(m, group_number, peer_number, chat);
  }
  ...
  memcpy(&chat->moderation.sanctions_creds, &creds,
         sizeof(struct GC_Sanction_Creds));
 ...
 * Returns -1 on failure.
 */
static int make_gc_mod_list_packet(const GC_Chat *chat, uint8_t *data,
                                   uint32_t maxlen, size_t mod_list_size) {
  ...
  if (mod_list_size > 0) {
    ...
    memcpy(data + HASH_ID_BYTES + sizeof(uint16_t), packed_mod_list,
           mod_list_size);
  }
...
static int send_peer_mod_list(GC_Chat *chat, GC_Connection *gconn) {
  ...
  int packet_len =
      make_gc_mod_list_packet(chat, packet, SIZEOF_VLA(packet), mod_list_size);
 ...
 * Returns -1 on failure.
 */
static int make_gc_sanctions_list_packet(GC_Chat *chat, uint8_t *data,
                                         uint32_t maxlen) {
  ...
  int packed_len = sanctions_list_pack(
      data + length, maxlen - length, chat->moderation.sanctions,
...
static int send_peer_sanctions_list(GC_Chat *chat, GC_Connection *gconn) {
  ...
  return send_lossless_group_packet(chat, gconn, packet, packet_len,
                                    GP_SANCTIONS_LIST);
}
...
static int broadcast_gc_sanctions_list(GC_Chat *chat) {
  ...
  send_gc_lossless_packet_all_peers(chat, packet, packet_len,
                                    GP_SANCTIONS_LIST);
  return 0;
}
 ...
 */
static int send_gc_self_exit(GC_Chat *chat, const uint8_t *partmessage,
                             uint32_t length) {
...
static int handle_gc_peer_exit(Messenger *m, int group_number,
                               uint32_t peer_number, const uint8_t *data,
                               uint32_t length) {
  if (length > MAX_GC_PART_MESSAGE_SIZE) {
 ...
 */
int gc_set_self_nick(Messenger *m, int group_number, const uint8_t *nick,
                     uint16_t length) {
  GC_Session *c = m->group_handler;
  GC_Chat *chat = gc_get_group(c, group_number);
...
uint8_t gc_get_self_role(const GC_Chat *chat) { return chat->group[0].role; }
...
int gc_get_peer_nick(const GC_Chat *chat, uint32_t peer_id, uint8_t *name) {
  ...
  if (name) {
    memcpy(name, chat->group[peer_number].nick,
           chat->group[peer_number].nick_length);
  }
...
static int handle_gc_nick(Messenger *m, int group_number, uint32_t peer_number,
                          const uint8_t *nick, uint32_t length) {
  GC_Session *c = m->group_handler;
  GC_Chat *chat = gc_get_group(c, group_number);
  ...
  if (length == 0 || length > MAX_GC_NICK_SIZE ||
      get_nick_peer_number(chat, nick, length) != -1) {
    return gc_peer_delete(m, group_number, peer_number, nullptr, 0, false);
  }

  if (c->nick_change) {
    (*c->nick_change)(m, group_number, chat->group[peer_number].peer_id, nick,
                      length, c->nick_change_userdata);
  }
 ...
 */
int gc_get_peer_public_key(const GC_Chat *chat, uint32_t peer_number,
                           uint8_t *public_key) {
  GC_Connection *gconn = gcc_get_connection(chat, peer_number);
...
/* Creates a topic packet and puts it in data. Packet includes the topic, topic
 * length, public signature key of the setter, topic version, and the signature.
 *
 * Returns packet length on success.
 * Returns -1 on failure.
 */
static int make_gc_topic_packet(GC_Chat *chat, uint8_t *data, uint16_t length) {
  if (length < HASH_ID_BYTES + SIGNATURE_SIZE + chat->topic_info.length +
                   GC_MIN_PACKED_TOPIC_INFO_SIZE) {
    return -1;
  }
  ...
  uint16_t packed_len = pack_gc_topic_info(
      data + data_length, length - data_length, &chat->topic_info);
...
static int send_peer_topic(GC_Chat *chat, GC_Connection *gconn) {
  VLA(uint8_t, packet,
      HASH_ID_BYTES + SIGNATURE_SIZE + chat->topic_info.length +
          GC_MIN_PACKED_TOPIC_INFO_SIZE);
  ...
  if (send_lossless_group_packet(chat, gconn, packet, packet_len, GP_TOPIC) ==
      -1) {
    return -1;
  }
...
/* Sets the group topic and broadcasts it to the group. Setter must be a
 * moderator or founder.
 *
 * Returns 0 on success.
 * Returns -1 if the topic is too long.
 * Returns -2 if the caller does not have the required permissions to set the
 * topic. Returns -3 if the packet cannot be created or signing fails. Returns
 * -4 if the packet fails to send.
 */
int gc_set_topic(GC_Chat *chat, const uint8_t *topic, uint16_t length) {
  ...
  if (chat->topic_info.version !=
      UINT32_MAX) { /* TODO (jfreegman) improbable, but an overflow would break
                       everything */
  ...
  memcpy(chat->topic_info.public_sig_key, get_sig_pk(chat->self_public_key),
         SIG_PUBLIC_KEY);

  int err = -3;
  VLA(uint8_t, packed_topic, length + GC_MIN_PACKED_TOPIC_INFO_SIZE);
  uint16_t packed_len = pack_gc_topic_info(
      packed_topic, SIZEOF_VLA(packed_topic), &chat->topic_info);

  if (packed_len != SIZEOF_VLA(packed_topic)) {
    goto ON_ERROR;
  }

  if (crypto_sign_detached(chat->topic_sig, nullptr, packed_topic, packed_len,
...
/* If public_sig_key is equal to the key of the topic setter, replaces topic
 * credentials and re-broadcast the updated topic info to the group.
 *
 * Returns 0 on success
 * Returns -1 on failure.
 */
static int update_gc_topic(GC_Chat *chat, const uint8_t *public_sig_key) {
  if (memcmp(public_sig_key, chat->topic_info.public_sig_key, SIG_PUBLIC_KEY) !=
      0) {
    return 0;
  }

  if (gc_set_topic(chat, chat->topic_info.topic, chat->topic_info.length) !=
      0) {
    return -1;
  }
...
static int handle_gc_topic(Messenger *m, int group_number, uint32_t peer_number,
                           const uint8_t *data, uint32_t length) {
  if (length >
      SIGNATURE_SIZE + MAX_GC_TOPIC_SIZE + GC_MIN_PACKED_TOPIC_INFO_SIZE) {
    return -1;
  }
  ...
  int unpacked_len = unpack_gc_topic_info(&topic_info, data + SIGNATURE_SIZE,
  ...
  if (crypto_sign_verify_detached(signature, data + SIGNATURE_SIZE,
                                  length - SIGNATURE_SIZE,
  ...
  bool skip_callback =
      chat->topic_info.length == topic_info.length &&
  ...
  if (!skip_callback && chat->connection_state == CS_CONNECTED &&
      c->topic_change) {
    (*c->topic_change)(m, group_number, chat->group[peer_number].peer_id,
                       topic_info.topic, topic_info.length,
...
void gc_get_group_name(const GC_Chat *chat, uint8_t *group_name) {
  if (group_name) {
    memcpy(group_name, chat->shared_state.group_name,
           chat->shared_state.group_name_len);
  }
}
...
void gc_get_password(const GC_Chat *chat, uint8_t *password) {
  if (password) {
    memcpy(password, chat->shared_state.password,
           chat->shared_state.password_length);
  }
}
 ...
 *
 * This function requires that the shared state be re-signed and will only work
 * for the group founder.
 *
 * Returns 0 on success.
 ...
 * Returns -3 if the packet failed to send.
 */
int gc_founder_set_password(GC_Chat *chat, const uint8_t *password,
...
static int handle_gc_set_mod(Messenger *m, int group_number,
                             uint32_t peer_number, const uint8_t *data,
                             uint32_t length) {
  ...
  if (c->moderation) {
    (*c->moderation)(m, group_number, chat->group[peer_number].peer_id,
                     chat->group[target_peernum].peer_id,
...
/* Adds or removes gconn from moderator list if add_mod is true or false
 * respectively. Re-signs and re-distributes an updated mod_list hash.
 *
 * Returns 0 on success.
 * Returns -1 on failure.
 */
static int founder_gc_set_moderator(GC_Chat *chat, GC_Connection *gconn,
                                    bool add_mod) {
  if (chat->group[0].role != GR_FOUNDER) {
    return -1;
  }
...
static int handle_gc_set_observer(Messenger *m, int group_number,
                                  uint32_t peer_number, const uint8_t *data,
                                  uint32_t length) {
  ...
  if (add_obs) {
    ...
    if (sanctions_list_unpack(&sanction, &creds, 1, data + 1 + EXT_PUBLIC_KEY,
                              length - 1 - EXT_PUBLIC_KEY, nullptr) != 1) {
      return -1;
    }
    ...
    if (sanctions_creds_unpack(&creds, data + 1 + EXT_PUBLIC_KEY,
                               length - 1 - EXT_PUBLIC_KEY) !=
 ...
 * Returns -1 on failure.
 */
static int send_gc_set_observer(GC_Chat *chat, GC_Connection *gconn,
                                const uint8_t *sanction_data, uint32_t length,
                                bool add_obs) {
...
/* Adds or removes peer_number from the observer list if add_obs is true or
 * false respectively. Broadcasts this change to the entire group.
 *
 * Returns 0 on success.
 * Returns -1 on failure.
 */
static int mod_gc_set_observer(GC_Chat *chat, uint32_t peer_number,
                               bool add_obs) {
  GC_Connection *gconn = gcc_get_connection(chat, peer_number);
  ...
  uint8_t sanction_data[sizeof(struct GC_Sanction) +
                        sizeof(struct GC_Sanction_Creds)];
  uint32_t length = 0;
  ...
  if (add_obs) {
    ...
    if (sanctions_list_make_entry(chat, peer_number, &sanction, SA_OBSERVER) ==
        -1) {
      fprintf(stderr,
              "sanctions_list_make_entry failed in mod_gc_set_observer\n");
      return -1;
    }

    int packed_len =
        sanctions_list_pack(sanction_data, sizeof(sanction_data), &sanction,
    ...
    if (sanctions_list_remove_observer(chat, gconn->addr.public_key, nullptr) ==
        -1) {
      return -1;
    }

    uint16_t packed_len =
        sanctions_creds_pack(&chat->moderation.sanctions_creds, sanction_data,
...
/* Sets the role of peer_number. role must be one of: GR_MODERATOR, GR_USER,
 * GR_OBSERVER
 *
 * Returns 0 on success.
 * Returns -1 if the group_number is invalid.
 ...
 */
int gc_set_peer_role(Messenger *m, int group_number, uint32_t peer_id,
                     uint8_t role) {
  GC_Session *c = m->group_handler;
  GC_Chat *chat = gc_get_group(c, group_number);
  ...
  if (chat->group[0].role != GR_FOUNDER &&
      (role == GR_MODERATOR || chat->group[peer_number].role <= GR_MODERATOR)) {
    return -3;
  }
...
/* Sets the group privacy state and distributes the new shared state to the
 * group.
 *
 * This function requires that the shared state be re-signed and will only work
 * for the group founder.
 ...
 * Returns -3 if the caller does not have sufficient permissions for this
 * action. Returns -4 if the group is disconnected. Returns -5 if the privacy
 * state could not be set. Returns -6 if the packet failed to send.
 */
int gc_founder_set_privacy_state(Messenger *m, int group_number,
                                 uint8_t new_privacy_state) {
  GC_Session *c = m->group_handler;
  GC_Chat *chat = gc_get_group(c, group_number);
...
/* Sets the peer limit to maxpeers and distributes the new shared state to the
 * group.
 *
 * This function requires that the shared state be re-signed and will only work
 * for the group founder.
 *
 * Returns 0 on success.
 * Returns -1 if the caller does not have sufficient permissions for this
 * action. Returns -2 if the peer limit could not be set. Returns -3 if the
 ...
 * Returns -5 if the packet fails to send.
 */
int gc_send_message(GC_Chat *chat, const uint8_t *message, uint16_t length,
                    uint8_t type) {
  ...
  uint8_t packet_type =
      type == GC_MESSAGE_TYPE_NORMAL ? GM_PLAIN_MESSAGE : GM_ACTION_MESSAGE;
...
static int handle_gc_message(Messenger *m, int group_number,
                             uint32_t peer_number, const uint8_t *data,
                             uint32_t length, uint8_t type) {
  ...
  if (chat->group[peer_number].ignore ||
      chat->group[peer_number].role >= GR_OBSERVER) {
    return 0;
  }
  ...
  unsigned int cb_type =
      (type == GM_PLAIN_MESSAGE) ? MESSAGE_NORMAL : MESSAGE_ACTION;

  if (c->message) {
    (*c->message)(m, group_number, chat->group[peer_number].peer_id, cb_type,
                  data, length, c->message_userdata);
  }
 ...
 * Returns -5 if the packet fails to send.
 */
int gc_send_private_message(GC_Chat *chat, uint32_t peer_id, uint8_t type,
  ...
  uint32_t packet_len = make_gc_broadcast_header(
      chat, message_with_type, length + 1, packet, GM_PRIVATE_MESSAGE);

  if (send_lossless_group_packet(chat, gconn, packet, packet_len,
                                 GP_BROADCAST) == -1) {
    return -6;
  }
...
static int handle_gc_private_message(Messenger *m, int group_number,
                                     uint32_t peer_number, const uint8_t *data,
                                     uint32_t length) {
  ...
  if (c->private_message) {
    (*c->private_message)(m, group_number, chat->group[peer_number].peer_id,
                          message_type, data + 1, length - 1,
                          c->private_message_userdata);
  }

  return 0;
}

/* Sends a custom packet to the group. If lossless is true, the packet will be
 * lossless.
 *
 * Returns 0 on success.
 * Returns -1 if the message is too long.
 ...
 */
int gc_send_custom_packet(GC_Chat *chat, bool lossless, const uint8_t *data,
                          uint32_t length) {
  if (length > MAX_GC_MESSAGE_SIZE) {
    return -1;
  }
 ...
 * Returns -1 on failure.
 */
static int handle_gc_custom_packet(Messenger *m, int group_number,
                                   uint32_t peer_number, const uint8_t *data,
                                   uint32_t length) {
  ...
  if (c->custom_packet) {
    (*c->custom_packet)(m, group_number, chat->group[peer_number].peer_id, data,
                        length, c->custom_packet_userdata);
  }

  return 0;
}

static int handle_gc_kick_peer(Messenger *m, int group_number,
                               uint32_t peer_number, const uint8_t *data,
                               uint32_t length) {
  ...
  if (target_peer_number == 0) {
    if (c->moderation) {
      (*c->moderation)(m, group_number, chat->group[peer_number].peer_id,
                       chat->group[target_peer_number].peer_id, mod_event,
  ...
  if (target_peer_number ==
      -1) { /* we don't need to/can't kick a peer that isn't in our peerlist */
    return 0;
  }
  ...
  if (gc_peer_delete(m, group_number, target_peer_number, nullptr, 0, false) ==
      -1) {
    return -1;
  }
 ...
 * Returns -2 if the peer_id is invalid.
 * Returns -3 if the caller does not have sufficient permissions for this
 * action. Returns -4 if the action failed. Returns -5 if the packet failed to
 * send. Returns -6 if the caller attempted to kick himself.
 */
int gc_kick_peer(Messenger *m, int group_number, uint32_t peer_id) {
  ...
  if (chat->group[0].role >= GR_USER ||
      chat->group[peer_number].role == GR_FOUNDER) {
    return -3;
  }

  if (chat->group[0].role != GR_FOUNDER &&
      chat->group[peer_number].role == GR_MODERATOR) {
    return -3;
  }
  ...
  if (chat->group[peer_number].role == GR_MODERATOR ||
      chat->group[peer_number].role == GR_OBSERVER) {
    /* this first removes peer from any lists they're on and broadcasts new
     * lists to group */
 ...
 */
int gc_send_message_ack(const GC_Chat *chat, GC_Connection *gconn,
                        uint64_t read_id, uint64_t request_id) {
...
/* If packet contains a non-zero request_id we try to resend its respective
 * packet. If packet contains a non-zero read_id we remove the packet from our
 * send array.
 *
 * Returns non-negative value on success.
 * Return -1 if error or we fail to send a packet in case of a request response.
 */
static int handle_gc_message_ack(GC_Chat *chat, GC_Connection *gconn,
                                 const uint8_t *data, uint32_t length) {
  ...
  if (gconn->send_array[idx].data == nullptr ||
      gconn->send_array[idx].data_length == 0) {
    return -1;
  }
  ...
  if (gconn->send_array[idx].message_id == request_id &&
      (gconn->send_array[idx].last_send_try != tm ||
       gconn->send_array[idx].time_added == tm)) {
    gconn->send_array[idx].last_send_try = tm;
    return sendpacket(chat->net, gconn->addr.ip_port,
                      gconn->send_array[idx].data,
 ...
 * Returns -1 on failure.
 */
static int handle_gc_hs_response_ack(Messenger *m, int group_number,
                                     GC_Connection *gconn) {
  GC_Chat *chat = gc_get_group(m->group_handler, group_number);
  ...
  if (gconn->friend_shared_state_version >
          gconn->self_sent_shared_state_version ||
      (gconn->friend_shared_state_version ==
           gconn->self_sent_shared_state_version &&
 ...
 * Returns -1 on failure.
 */
static int handle_gc_broadcast(Messenger *m, int group_number,
                               uint32_t peer_number, const uint8_t *data,
                               uint32_t length) {
  ...
  case GM_PLAIN_MESSAGE:
    return handle_gc_message(m, group_number, peer_number, message, m_len,
                             broadcast_type);

  case GM_PRIVATE_MESSAGE:
    return handle_gc_private_message(m, group_number, peer_number, message,
                                     m_len);
  ...
  default:
    fprintf(
        stderr,
        "Warning: handle_gc_broadcast received an invalid broadcast type %u\n",
        broadcast_type);
    return -1;
  }
}
 ...
 * Returns -1 on failure.
 */
static int uwrap_group_handshake_packet(const uint8_t *self_sk,
                                        uint8_t *sender_pk, uint8_t *plain,
                                        size_t plain_size,
                                        const uint8_t *packet,
                                        uint16_t length) {
  if (plain_size < length - 1 - HASH_ID_BYTES - ENC_PUBLIC_KEY -
  ...
  int plain_len = decrypt_data(
      sender_pk, self_sk, nonce,
...
/* Encrypts data of length using the peer's shared key a new nonce. Packet must
 * have room for GC_MIN_ENCRYPTED_HS_PACKET_SIZE bytes.
 *
 * Adds plaintext header consisting of: packet identifier, chat_id_hash, self
 * public key, nonce.
 *
 * Returns length of encrypted packet on success.
 * Returns -1 on failure.
 */
static int wrap_group_handshake_packet(const uint8_t *self_pk,
                                       const uint8_t *self_sk,
  ...
  memcpy(packet + 1 + HASH_ID_BYTES + ENC_PUBLIC_KEY + CRYPTO_NONCE_SIZE,
         encrypt, enc_len);

  return 1 + HASH_ID_BYTES + ENC_PUBLIC_KEY + CRYPTO_NONCE_SIZE + enc_len;
}

/* Makes, wraps and encrypts a group handshake packet (both request and response
 * are the same format).
 *
 * Packet contains the handshake header, the handshake type, self pk hash,
 * session pk, self public signature key, the request type
 * (Group_Handshake_Request_Type), the join type (GROUP_HANDSHAKE_JOIN_TYPE),
...
static int make_gc_handshake_packet(GC_Chat *chat, GC_Connection *gconn,
  ...
  uint32_t state = gconn->self_sent_shared_state_version != UINT32_MAX
                       ? gconn->self_sent_shared_state_version
                       : chat->connection_state == CS_CONNECTED
                             ? chat->shared_state.version
                             : 0;
  ...
  int nodes_size =
      pack_nodes(data + length, sizeof(Node_format), node, MAX_SENT_GC_NODES);
  ...
  int enc_len = wrap_group_handshake_packet(
      chat->self_public_key, chat->self_secret_key, gconn->addr.public_key,
      packet, packet_size, data, length, chat->chat_id_hash);
...
static int send_gc_handshake_packet(GC_Chat *chat, uint32_t peer_number,
  ...
  int length =
      make_gc_handshake_packet(chat, gconn, handshake_type, request_type,
                               join_type, packet, sizeof(packet), node);
  ...
  ret2 = send_packet_tcp_connection(chat->tcp_conn, gconn->tcp_connection_num,
                                    packet, length);
...
static int send_gc_oob_handshake_packet(GC_Chat *chat, uint32_t peer_number,
  ...
  return tcp_send_oob_packet_using_relay(chat->tcp_conn, gconn->oob_relay_pk,
                                         gconn->addr.public_key, packet,
                                         length);
}

/* Handles a handshake response packet and takes appropriate action depending on
 * the value of request_type.
 *
 * Returns peer_number of new connected peer on success.
 * Returns -1 on failure.
 */
static int handle_gc_handshake_response(Messenger *m, int group_number,
                                        const uint8_t *sender_pk,
                                        const uint8_t *data, uint16_t length) {
  ...
  encrypt_precompute(sender_session_pk, gconn->session_secret_key,
                     gconn->shared_key);
  ...
  case HS_INVITE_REQUEST:
    net_unpack_u32(data + ENC_PUBLIC_KEY + SIG_PUBLIC_KEY + 2,
                   &gconn->friend_shared_state_version);
  ...
  default:
    fprintf(stderr, "Warning: received invalid request type in "
                    "handle_gc_handshake_response\n");
    return -1;
  }
...
static int send_gc_handshake_response(GC_Chat *chat, uint32_t peer_number,
                                      uint8_t request_type) {
  if (send_gc_handshake_packet(chat, peer_number, GH_RESPONSE, request_type,
                               0) == -1) {
    return -1;
  }
...
static int peer_reconnect(Messenger *m, const GC_Chat *chat,
...
static int handle_gc_handshake_request(Messenger *m, int group_number,
                                       IP_Port *ipp, const uint8_t *sender_pk,
                                       const uint8_t *data, uint32_t length) {
  ...
  int nodes_count = unpack_nodes(node, GCA_MAX_ANNOUNCED_TCP_RELAYS, nullptr,
  ...
  if (join_type == HJ_PUBLIC && !is_public_chat(chat)) {
    gc_peer_delete(m, group_number, peer_number, (const uint8_t *)"Join priv",
                   9, false);
    return -1;
  }
 ...
 * Returns -1 on failure.
 */
static int handle_gc_handshake_packet(Messenger *m, GC_Chat *chat, IP_Port *ipp,
                                      const uint8_t *packet, uint16_t length,
                                      bool direct_conn) {
  ...
  VLA(uint8_t, data,
      length - 1 - HASH_ID_BYTES - ENC_PUBLIC_KEY - CRYPTO_NONCE_SIZE -
          CRYPTO_MAC_SIZE);

  int plain_len = uwrap_group_handshake_packet(
      chat->self_secret_key, sender_pk, data, SIZEOF_VLA(data), packet, length);
  ...
  if (handshake_type == GH_REQUEST) {
    peer_number = handle_gc_handshake_request(m, chat->group_number, ipp,
                                              sender_pk, real_data, real_len);
  } else if (handshake_type == GH_RESPONSE) {
    peer_number = handle_gc_handshake_response(m, chat->group_number, sender_pk,
                                               real_data, real_len);
  } else {
    return -1;
  }
...
int handle_gc_lossless_helper(Messenger *m, int group_number,
                              uint32_t peer_number, const uint8_t *data,
                              uint16_t length, uint64_t message_id,
                              uint8_t packet_type) {
  GC_Session *c = m->group_handler;
  GC_Chat *chat = gc_get_group(c, group_number);
  ...
  case GP_PEER_INFO_RESPONSE:
    return handle_gc_peer_info_response(m, group_number, peer_number, data,
                                        length);

  case GP_SYNC_REQUEST:
    return handle_gc_sync_request(m, group_number, peer_number, gconn, data,
                                  length);

  case GP_SYNC_RESPONSE:
    return handle_gc_sync_response(m, group_number, peer_number, gconn, data,
                                   length);
 ...
 * return -1 on failure.
 */
static int handle_gc_lossless_message(Messenger *m, GC_Chat *chat,
                                      const uint8_t *packet, uint16_t length,
                                      bool direct_conn) {
  ...
  int len = unwrap_group_packet(gconn->shared_key, data, &message_id,
  ...
  if (packet_type != GP_HS_RESPONSE_ACK && packet_type != GP_INVITE_REQUEST &&
      !gconn->handshaked) {
  ...
  if (message_id == 3 && is_invite_packet && gconn->received_message_id == 1) {
    // probably we missed initial handshake packet. update received packets
    // index
    // TODO: FIXME
    ++gconn->received_message_id;
  }

  int lossless_ret = gcc_handle_received_message(
      chat, peer_number, real_data, real_len, packet_type, message_id);

  if (packet_type == GP_INVITE_REQUEST &&
      !gconn->handshaked) { // race condition
  ...
  int ret =
      handle_gc_lossless_helper(m, chat->group_number, peer_number, real_data,
                                real_len, message_id, packet_type);
 ...
 * return -1 on failure.
 */
static int handle_gc_lossy_message(Messenger *m, GC_Chat *chat,
                                   const uint8_t *packet, uint16_t length,
                                   bool direct_conn) {
  ...
  int len = unwrap_group_packet(gconn->shared_key, data, nullptr, &packet_type,
                                packet, length);
  ...
  case GP_INVITE_RESPONSE_REJECT:
    ret =
        handle_gc_invite_response_reject(m, chat->group_number, real_data, len);
    break;
  ...
  case GP_CUSTOM_PACKET:
    ret = handle_gc_custom_packet(m, chat->group_number, peer_number, real_data,
                                  len);
    break;
...
static bool group_can_handle_packets(GC_Chat *chat) {
  return chat->connection_state != CS_FAILED &&
         chat->connection_state != CS_MANUALLY_DISCONNECTED;
}
 ...
 * Returns -1 on failure.
 */
static int handle_gc_tcp_packet(void *object, int id, const uint8_t *packet,
...
static int handle_gc_tcp_oob_packet(void *object, const uint8_t *public_key,
  ...
  if (handle_gc_handshake_packet(m, chat, nullptr, packet, length, false) ==
      -1) {
    return -1;
  }
...
static int handle_gc_udp_packet(void *object, IP_Port ipp,
                                const uint8_t *packet, uint16_t length,
                                void *userdata) {
  if (length <= 1 + sizeof(uint32_t)) {
    return -1;
  ...
  if (!chat) {
    fprintf(stderr,
            "get_chat_by_hash failed in handle_gc_udp_packet (type %u)\n",
            packet[0]);
    return -1;
  }
...
/*
 * Deletes peer_number from group. `no_callback` should be set to true if the
 * `peer_exit` callback should not be triggered.
 *
 * Return 0 on success.
 * Return -1 on failure.
 */
int gc_peer_delete(Messenger *m, int group_number, uint32_t peer_number,
                   const uint8_t *data, uint16_t length, bool no_callback) {
  GC_Session *c = m->group_handler;
  ...
  if (gconn->handshaked && !is_peer_confirmed(chat, gconn->addr.public_key)) {
    memcpy(chat->confirmed_peers[chat->confirmed_peers_index],
           gconn->addr.public_key, ENC_PUBLIC_KEY);
    chat->confirmed_peers_index =
        (chat->confirmed_peers_index + 1) % MAX_GC_CONFIRMED_PEERS;
  }

  /* Needs to occur before peer is removed*/
  if (!no_callback && c->peer_exit && gconn->confirmed) {
    (*c->peer_exit)(m, group_number, chat->group[peer_number].peer_id,
                    chat->group[peer_number].nick,
                    chat->group[peer_number].nick_length, data, length,
                    c->peer_exit_userdata);
  }
  ...
  if (chat->numpeers != peer_number) {
    memcpy(&chat->group[peer_number], &chat->group[chat->numpeers],
           sizeof(GC_GroupPeer));
    memcpy(&chat->gcc[peer_number], &chat->gcc[chat->numpeers],
           sizeof(GC_Connection));
  }

  memset(&chat->group[chat->numpeers], 0, sizeof(GC_GroupPeer));
  memset(&chat->gcc[chat->numpeers], 0, sizeof(GC_Connection));

  GC_GroupPeer *tmp_group = (GC_GroupPeer *)realloc(
      chat->group, sizeof(GC_GroupPeer) * chat->numpeers);
  ...
  GC_Connection *tmp_gcc = (GC_Connection *)realloc(
      chat->gcc, sizeof(GC_Connection) * chat->numpeers);
...
static int peer_update(Messenger *m, int group_number, GC_GroupPeer *peer,
  ...
  if ((nick_num != -1 && nick_num != peer_number)) { /* duplicate nick */
    if (c->peer_exit) {
      (*c->peer_exit)(m, group_number, chat->group[peer_number].peer_id,
                      chat->group[peer_number].nick,
                      chat->group[peer_number].nick_length, nullptr, 0,
                      c->peer_exit_userdata);
    }

    gc_peer_delete(m, group_number, peer_number, (const uint8_t *)"dup nick", 8,
                   false);
    return -1;
  }
...
static int peer_add(Messenger *m, int group_number, IP_Port *ipp,
  ...
  GC_Connection *tmp_gcc = (GC_Connection *)realloc(
      chat->gcc, sizeof(GC_Connection) * (chat->numpeers + 1));
  ...
  GC_GroupPeer *tmp_group = (GC_GroupPeer *)realloc(
      chat->group, sizeof(GC_GroupPeer) * (chat->numpeers + 1));
  ...
  memcpy(gconn->addr.public_key, public_key,
         ENC_PUBLIC_KEY); /* we get the sig key in the handshake */

  gconn->public_key_hash = get_peer_key_hash(public_key);
  gconn->last_received_ping_time =
      mono_time_get(chat->mono_time) + (rand() % GC_PING_INTERVAL);
 ...
 */
static bool peer_timed_out(const Mono_Time *mono_time, const GC_Chat *chat,
                           GC_Connection *gconn) {
  return mono_time_is_timeout(mono_time, gconn->last_received_ping_time,
                              gconn->confirmed ? GC_CONFIRMED_PEER_TIMEOUT
...
static void do_peer_connections(Messenger *m, int group_number) {
  ...
  for (i = 1; i < chat->numpeers; ++i) {
    if (chat->gcc[i].confirmed) {
      if (mono_time_is_timeout(m->mono_time,
                               chat->gcc[i].last_tcp_relays_shared,
                               GCC_TCP_SHARED_RELAYS_TIMEOUT)) {
...
static void ping_group(GC_Chat *chat) {
  if (!mono_time_is_timeout(chat->mono_time, chat->last_sent_ping_time,
                            GC_PING_INTERVAL)) {
    return;
  }
  ...
  net_pack_u32(data + HASH_ID_BYTES + sizeof(uint32_t),
               chat->shared_state.version);
  net_pack_u32(data + HASH_ID_BYTES + (sizeof(uint32_t) * 2),
               chat->moderation.sanctions_creds.version);
  net_pack_u32(data + HASH_ID_BYTES + (sizeof(uint32_t) * 3),
               chat->topic_info.version);
...
static int send_pending_handshake(GC_Chat *chat, GC_Connection *gconn,
                                  uint32_t peer_number) {
  ...
  if (gconn->is_pending_handshake_response) {
    result = send_gc_handshake_response(chat, peer_number,
  ...
  if (!result || time > gconn->pending_handshake +
                            PENDING_HANDSHAKE_SENDING_MAX_INTERVAL) {
    gconn->pending_handshake = 0;
  }
...
static void do_group_tcp(GC_Chat *chat, void *userdata) {
  ...
  for (i = 1; i < chat->numpeers; ++i) {
    ...
    set_tcp_connection_to_status(chat->tcp_conn, gconn->tcp_connection_num,
                                 tcp_set);
...
/* CS_CONNECTED: Peers are pinged, unsent packets are resent, and timeouts are
 * checked. CS_CONNECTING: Look for new DHT nodes after an interval.
 * CS_DISCONNECTED: Send an invite request using a random node if our timeout
 * GROUP_JOIN_ATTEMPT_INTERVAL has expired. CS_FAILED: Do nothing. This occurs
 * if we cannot connect to a group or our invite request is rejected.
 */
void do_gc(GC_Session *c, void *userdata) {
  ...
  for (i = 0; i < c->num_chats; ++i) {
    ...
    case CS_CONNECTING: {
      if (mono_time_is_timeout(chat->mono_time, chat->last_join_attempt,
                               GROUP_JOIN_ATTEMPT_INTERVAL)) {
        chat->connection_state = CS_DISCONNECTED;
      }
    ...
    case CS_DISCONNECTED: {
      ...
      int tcp_num = tcp_copy_connected_relays(chat->tcp_conn, tcp_relays,
                                              TCP_RELAYS_TEST_COUNT);
      ...
      for (j = 1; j < chat->numpeers; ++j) {
        ...
        if (!gconn->handshaked && !gconn->pending_handshake) {
          gconn->pending_handshake = mono_time_get(c->messenger->mono_time) +
                                     HANDSHAKE_SENDING_TIMEOUT;
        }
      }
...
static int get_new_group_index(GC_Session *c) {
  ...
  memset(&(c->chats[new_index].saved_invites), -1,
         MAX_GC_SAVED_INVITES * sizeof(uint32_t));
...
static GC_Chat *get_chat_by_tcp_connections(GC_Session *group_handler,
...
static void add_tcp_relays_to_chat(Messenger *m, GC_Chat *chat) {
  ...
  unsigned int i, num = tcp_copy_connected_relays(nc_get_tcp_c(m->net_crypto),
                                                  tcp_relays, num_relays);

  for (i = 0; i < num; ++i) {
    add_tcp_relay_global(chat->tcp_conn, tcp_relays[i].ip_port,
                         tcp_relays[i].public_key);
  }
}

static int init_gc_tcp_connection(Messenger *m, GC_Chat *chat) {
  chat->tcp_conn = new_tcp_connections(m->mono_time, chat->self_secret_key,
  ...
  set_oob_packet_tcp_connection_callback(chat->tcp_conn,
                                         &handle_gc_tcp_oob_packet, m);
  set_connection_status_updated_callback(
      chat->tcp_conn, &handle_connection_status_updated_callback, m);

  return 0;
}

static int create_new_group(GC_Session *c, const uint8_t *nick,
                            size_t nick_length, bool founder) {
  ...
  if (peer_add(m, group_number, nullptr, chat->self_public_key) !=
      0) { /* you are always peer_number/index 0 */
    group_delete(c, chat);
    return -1;
  }
...
/* Initializes group shared state and creates a signature for it using the chat
 * secret key.
 *
 * Return 0 on success.
 * Return -1 on failure.
 */
static int init_gc_shared_state(GC_Chat *chat, uint8_t privacy_state,
                                const uint8_t *group_name,
                                uint16_t name_length) {
  memcpy(chat->shared_state.founder_public_key, chat->self_public_key,
         EXT_PUBLIC_KEY);
...
/* Inits the sanctions list credentials. This should be called by the group
 * founder on creation.
 *
 * Returns 0 on success.
 * Returns -1 on failure.
...
static void gc_load_peers(Messenger *m, GC_Chat *chat, GC_SavedPeerInfo *addrs,
                          uint16_t num_addrs) {
  int i;
  ...
  for (i = 0; i < num_addrs && i < MAX_GC_PEER_ADDRS; ++i) {
    ...
    int peer_number =
        peer_add(m, chat->group_number, ip_port, addrs[i].public_key);
    ...
    add_tcp_relay_global(chat->tcp_conn, addrs[i].tcp_relay.ip_port,
                         addrs[i].tcp_relay.public_key);

    int add_tcp_result = add_tcp_relay_connection(
        chat->tcp_conn, gconn->tcp_connection_num, addrs[i].tcp_relay.ip_port,
        addrs[i].tcp_relay.public_key);
...
int gc_group_load(GC_Session *c, Saved_Group *save, int group_number) {
  ...
  chat->numpeers = 0;
  chat->connection_state =
      is_active_chat ? CS_CONNECTING : CS_MANUALLY_DISCONNECTED;
  ...
  memcpy(chat->shared_state.founder_public_key, save->founder_public_key,
         EXT_PUBLIC_KEY);
  chat->shared_state.group_name_len = net_ntohs(save->group_name_length);
  memcpy(chat->shared_state.group_name, save->group_name,
         MAX_GC_GROUP_NAME_SIZE);
  ...
  memcpy(chat->shared_state.mod_list_hash, save->mod_list_hash,
         GC_MODERATION_HASH_SIZE);
  ...
  memcpy(chat->topic_info.public_sig_key, save->topic_public_sig_key,
         SIG_PUBLIC_KEY);
  ...
  if (mod_list_unpack(chat, save->mod_list, num_mods * GC_MOD_LIST_ENTRY_SIZE,
                      num_mods) == -1) {
    return -1;
  }
 ...
 */
int gc_group_add(GC_Session *c, uint8_t privacy_state,
  ...
  if (init_gc_shared_state(chat, privacy_state, group_name,
                           group_name_length) == -1) {
    group_delete(c, chat);
    return -5;
  }
 ...
 *
 * If the group is not password protected password should be set to NULL and
 * password_length should be 0.
 *
 * Return group_number on success.
 * Return -1 if the group object fails to initialize.
 * Return -2 if chat_id is NULL or a group with chat_id already exists in the
 * chats array. Return -3 if nick is too long. Return -4 if nick is empty or
 * nick length is zero. Return -5 if there is an error setting the group
 * password. Return -6 if there is an error adding a friend.
 */
int gc_group_join(GC_Session *c, const uint8_t *chat_id, const uint8_t *nick,
                  size_t nick_length, const uint8_t *passwd,
                  uint16_t passwd_len) {
  if (chat_id == nullptr || group_exists(c, chat_id) ||
      getfriend_id(c->messenger, chat_id) != -1) {
    return -2;
  }
...
static bool gc_rejoin_connected_group(GC_Session *c, GC_Chat *chat) {
  ...
  /* Remove all peers except self. Numpeers decrements with each call to
   * gc_peer_delete */
...
int gc_invite_friend(GC_Session *c, GC_Chat *chat, int32_t friend_number,
  ...
  chat->saved_invites_index =
      (chat->saved_invites_index + 1) % MAX_GC_SAVED_INVITES;

  return 0;
}

static int send_gc_invite_accepted_packet(Messenger *m, GC_Chat *chat,
...
static int send_gc_invite_confirmed_packet(Messenger *m, GC_Chat *chat,
                                           uint32_t friend_number,
                                           const uint8_t *data,
                                           uint16_t length) {
  if (!friend_is_valid(m, friend_number)) {
...
static bool copy_ip_port_to_gconn(Messenger *m, int friend_number,
                                  GC_Connection *gconn) {
...
int handle_gc_invite_confirmed_packet(GC_Session *c, int friend_number,
                                      const uint8_t *data, uint32_t length) {
  ...
  Node_format tcp_relays[GCC_MAX_TCP_SHARED_RELAYS];
  int num_nodes = unpack_nodes(tcp_relays, GCC_MAX_TCP_SHARED_RELAYS, nullptr,
                               data + ENC_PUBLIC_KEY + CHAT_ID_SIZE,
                               length - GC_JOIN_DATA_LENGTH, 1);

  bool copy_ip_port_result =
      copy_ip_port_to_gconn(c->messenger, friend_number, gconn);
...
int handle_gc_invite_accepted_packet(GC_Session *c, int friend_number,
                                     const uint8_t *data, uint32_t length) {
  if (length < GC_JOIN_DATA_LENGTH) {
    return -1;
  }
  ...
  if (num > 0) {
    ...
    int nodes_len =
        pack_nodes(send_data + len, sizeof(send_data) - len, tcp_relays, num);
 ...
 */
int gc_accept_invite(GC_Session *c, int32_t friend_number, const uint8_t *data,
                     uint16_t length, const uint8_t *nick, size_t nick_length,
...
GC_Session *new_dht_groupchats(Messenger *m) {
  ...
  networking_registerhandler(m->net, NET_PACKET_GC_LOSSLESS,
                             &handle_gc_udp_packet, m);
  networking_registerhandler(m->net, NET_PACKET_GC_LOSSY, &handle_gc_udp_packet,
                             m);
  networking_registerhandler(m->net, NET_PACKET_GC_HANDSHAKE,
                             &handle_gc_udp_packet, m);
 ...
 */
int gc_group_exit(GC_Session *c, GC_Chat *chat, const uint8_t *message,
                  uint16_t length) {
...
void kill_dht_groupchats(GC_Session *c) {
  ...
  networking_registerhandler(c->messenger->net, NET_PACKET_GC_LOSSY, nullptr,
                             nullptr);
  networking_registerhandler(c->messenger->net, NET_PACKET_GC_LOSSLESS, nullptr,
                             nullptr);
  networking_registerhandler(c->messenger->net, NET_PACKET_GC_HANDSHAKE,
                             nullptr, nullptr);
...
uint32_t gc_count_groups(const GC_Session *c) {
  ...
  for (i = 0; i < c->num_chats; ++i) {
    if (c->chats[i].connection_state > CS_NONE &&
        c->chats[i].connection_state < CS_INVALID) {
      ++count;
    }
  }
...
GC_Chat *gc_get_group_by_public_key(const GC_Session *c,
                                    const uint8_t *public_key) {
  int i;

  for (i = 0; i < c->num_chats; ++i) {
    if (!memcmp(public_key, get_chat_id(c->chats[i].chat_public_key),
                CHAT_ID_SIZE)) {
      return &c->chats[i];
    }
  }
...
static int get_nick_peer_number(const GC_Chat *chat, const uint8_t *nick,
  ...
  for (i = 0; i < chat->numpeers; ++i) {
    if (chat->group[i].nick_length == length &&
        memcmp(chat->group[i].nick, nick, length) == 0) {
      return i;
    }
  }
...
static bool group_exists(const GC_Session *c, const uint8_t *chat_id) {
  ...
  for (i = 0; i < c->num_chats; ++i) {
    if (memcmp(get_chat_id(c->chats[i].chat_public_key), chat_id,
               CHAT_ID_SIZE) == 0) {
      return true;
    }
  }
...
int add_peers_from_announces(const GC_Session *gc_session, GC_Chat *chat,
                             GC_Announce *announces,
  ...
  for (i = 0; i < gc_announces_count; ++i) {
    ...
    for (j = 0; j < tcp_relays_count; ++j) {
      ...
      int save_tcp_result =
          save_tcp_relay(gconn, &curr_announce->tcp_relays[j]);

      if (save_tcp_result) {
        continue;
      }

      memcpy(gconn->oob_relay_pk, curr_announce->tcp_relays[j].public_key,
             ENC_PUBLIC_KEY);
    }
