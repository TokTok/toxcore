...
TEST(PingArray, AddedDataCanBeChecked) {
...
TEST(PingArray, CheckWithDataTooSmallHasNoEffect) {
  Ping_Array_Ptr const arr(ping_array_new(2, 1));
  Mono_Time_Ptr const mono_time(mono_time_new());
...
TEST(PingArray, DataCanOnlyBeRetrievedOnce) {
  Ping_Array_Ptr arr(ping_array_new(2, 1));
  Mono_Time_Ptr mono_time(mono_time_new());
...
TEST(PingArray, PingIdMustMatch) {
  Ping_Array_Ptr arr(ping_array_new(1, 1));
  Mono_Time_Ptr mono_time(mono_time_new());
