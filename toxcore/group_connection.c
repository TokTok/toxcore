 ...
 * Return -1 on failure.
 */
static int create_array_entry(const Mono_Time *mono_time,
 ...
 */
int gcc_add_to_send_array(const Mono_Time *mono_time, GC_Connection *gconn,
                          const uint8_t *data, uint32_t length,
                          uint8_t packet_type) {
  /* check if send_array is full */
  if ((gconn->send_message_id % GCC_BUFFER_SIZE) ==
      (uint16_t)(gconn->send_array_start - 1)) {
    return -1;
  }
  ...
  if (create_array_entry(mono_time, array_entry, data, length, packet_type,
...
int gcc_handle_ack(GC_Connection *gconn, uint64_t message_id) {
  ...
  if (array_entry->message_id !=
      message_id) { // wrap-around indicates a connection problem
    return -1;
  }
 ...
 * Return -1 on failure
 */
int gcc_handle_received_message(GC_Chat *chat, uint32_t peer_number,
                                const uint8_t *data, uint32_t length,
                                uint8_t packet_type, uint64_t message_id) {
  ...
  /* we're missing an older message from this peer so we store it in
   * received_array */
  if (message_id > gconn->received_message_id + 1) {
    uint16_t idx = gcc_get_array_index(message_id);
    ...
    if (create_array_entry(chat->mono_time, ary_entry, data, length,
                           packet_type, message_id) == -1) {
      return -1;
    }
...
/* Handles peer_number's array entry with appropriate handler and clears it from
 * array.
 *
 * Return 0 on success.
 * Return -1 on failure.
 */
static int
process_received_array_entry(GC_Chat *chat, Messenger *m, int group_number,
  ...
  int ret = handle_gc_lossless_helper(
      m, group_number, peer_number, array_entry->data, array_entry->data_length,
...
/* Checks for and handles messages that are in proper sequence in gconn's
 * received_array. This should always be called after a new packet is handled in
 * correct sequence.
 *
 * Return 0 on success.
 * Return -1 on failure.
 */
int gcc_check_received_array(Messenger *m, int group_number,
                             uint32_t peer_number) {
  ...
  while (!array_entry_is_empty(array_entry)) {
    if (process_received_array_entry(chat, m, group_number, peer_number,
                                     array_entry) == -1) {
      return -1;
    }
...
void gcc_resend_packets(Messenger *m, GC_Chat *chat, uint32_t peer_number) {
  ...
  uint16_t i, start = gconn->send_array_start,
              end = gconn->send_message_id % GCC_BUFFER_SIZE;
  ...
  for (i = start; i != end; i = (i + 1) % GCC_BUFFER_SIZE) {
    ...
    if (delta > 1 && is_power_of_2(delta)) {
      gcc_send_group_packet(chat, gconn, array_entry->data,
                            array_entry->data_length, array_entry->packet_type);
      continue;
    }

    if (mono_time_is_timeout(m->mono_time, array_entry->time_added,
                             GC_CONFIRMED_PEER_TIMEOUT)) {
      gc_peer_delete(m, chat->group_number, peer_number,
                     (const uint8_t *)"Peer timed out", 14, false);
      return;
    }
  }
}
 ...
 * Returns -1 on failure.
 */
int gcc_send_group_packet(const GC_Chat *chat, const GC_Connection *gconn,
  ...
  if (!net_family_is_unspec(gconn->addr.ip_port.ip.family)) {
    if (gcc_connection_is_direct(chat->mono_time, gconn)) {
      if ((uint16_t)sendpacket(chat->net, gconn->addr.ip_port, packet,
                               length) == length) {
        return 0;
      }
  ...
  int ret = send_packet_tcp_connection(
      chat->tcp_conn, gconn->tcp_connection_num, packet, length);
...
bool gcc_connection_is_direct(const Mono_Time *mono_time,
                              const GC_Connection *gconn) {
  return ((GCC_UDP_DIRECT_TIMEOUT + gconn->last_received_direct_time) >
