...
Friend_Connections *new_friend_connections(Onion_Client *onion_c)
{
    ...
    if (temp->dht->local_discovery_enabled) {
...
{
    if (fr_c->last_LANdiscovery + LAN_DISCOVERY_INTERVAL < unix_time() && fr_c->dht->local_discovery_enabled) {
...
void kill_friend_connections(Friend_Connections *fr_c)
{
    ...
    if (fr_c->dht->local_discovery_enabled) {
