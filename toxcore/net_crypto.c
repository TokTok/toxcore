...
typedef enum Crypto_Conn_State {
  CRYPTO_CONN_FREE = 0, /* the connection slot is free; this value is 0 so it is
                         * valid after crypto_memzero(...) of the parent struct
                         */
  ...
  CRYPTO_CONN_NOT_CONFIRMED,     /* we are sending handshake packets; // AKE:
                                  * accepted     we have received one from the other,
                                  * but no data */
  CRYPTO_CONN_ESTABLISHED,
  /* the connection is established */ // AKE: confirmed
} Crypto_Conn_State;

typedef struct Crypto_Connection {
  uint8_t
      public_key[CRYPTO_PUBLIC_KEY_SIZE]; /* The real public key of the peer. */
  ...
  //	uint8_t sessionpublic_key[CRYPTO_PUBLIC_KEY_SIZE]; /* Our public key for
  //this session. */ 	uint8_t sessionsecret_key[CRYPTO_SECRET_KEY_SIZE]; /* Our
  //private key for this session. */ 	uint8_t
  //peersessionpublic_key[CRYPTO_PUBLIC_KEY_SIZE]; /* The session public key of
  //the peer. */ AKE NEW: This shared_key should be hashed (HKDF) => done
  // automatically in Noise => saved in handshake object AKE NEW: shared_key
  // still necessary for cookie phase / DHT-based shared_key
  uint8_t shared_key[CRYPTO_SHARED_KEY_SIZE]; /* The precomputed shared key from
                                                 encrypt_precompute. */
  Crypto_Conn_State status;       /* See Crypto_Conn_State documentation */
  uint64_t cookie_request_number; /* number used in the cookie request packets
                                     for this connection (echoID!) */
  uint8_t dht_public_key[CRYPTO_PUBLIC_KEY_SIZE]; /* The dht public key of the
                                                     peer */
  ...
  uint8_t *temp_packet; /* Where the cookie request/handshake packet is stored
                           while it is being sent. */
  uint16_t temp_packet_length;
  uint64_t temp_packet_sent_time; /* The time at which the last temp_packet was
                                     sent in ms. */
  uint32_t temp_packet_num_sent;

  IP_Port ip_portv4; /* The ip and port to contact this guy directly.*/
  IP_Port ip_portv6;
  uint64_t direct_lastrecv_timev4; /* The Time at which we last received a
                                      direct packet in ms. */
...
TCP_Connections *nc_get_tcp_c(const Net_Crypto *c) { return c->tcp_c; }

DHT *nc_get_dht(const Net_Crypto *c) { return c->dht; }

static uint8_t crypt_connection_id_not_valid(const Net_Crypto *c,
  ...
  const Crypto_Conn_State status =
      c->crypto_connections[crypt_connection_id].status;
...
#define COOKIE_LENGTH                                                          \
  (uint16_t)(CRYPTO_NONCE_SIZE + COOKIE_CONTENTS_LENGTH + CRYPTO_MAC_SIZE)

#define COOKIE_REQUEST_PLAIN_LENGTH                                            \
  (uint16_t)(COOKIE_DATA_LENGTH + sizeof(uint64_t))
#define COOKIE_REQUEST_LENGTH                                                  \
  (uint16_t)(1 + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_NONCE_SIZE +                  \
             COOKIE_REQUEST_PLAIN_LENGTH + CRYPTO_MAC_SIZE)
#define COOKIE_RESPONSE_LENGTH                                                 \
  (uint16_t)(1 + CRYPTO_NONCE_SIZE + COOKIE_LENGTH + sizeof(uint64_t) +        \
             CRYPTO_MAC_SIZE)
...
static int create_cookie_request(const Net_Crypto *c, uint8_t *packet,
                                 uint8_t *dht_public_key, uint64_t number,
                                 uint8_t *shared_key) {
  ...
  // AKE: shared key in case of cookie request is calculated by using initiators
  // DHT secret key and receivers DHT public key
...
/* Create cookie of length COOKIE_LENGTH from bytes of length COOKIE_DATA_LENGTH
 ...
 * return 0 on success.
 */
static int create_cookie(const Logger *log, const Mono_Time *mono_time,
                         uint8_t *cookie, const uint8_t *bytes,
  ...
  // AKE: "cookie" contains the Nonce used to encrypt the encrypted part of the
  // cookie
  random_nonce(cookie);
  int len =
      encrypt_data_symmetric(encryption_key, cookie, contents, sizeof(contents),
                             cookie + CRYPTO_NONCE_SIZE);
...
/* Open cookie of length COOKIE_LENGTH to bytes of length COOKIE_DATA_LENGTH
 * using encryption_key
 *
 ...
 * return 0 on success.
 */
static int open_cookie(const Logger *log, const Mono_Time *mono_time,
                       uint8_t *bytes, const uint8_t *cookie,
                       const uint8_t *encryption_key) {
  uint8_t contents[COOKIE_CONTENTS_LENGTH];
  const int len =
      decrypt_data_symmetric(encryption_key, cookie, cookie + CRYPTO_NONCE_SIZE,
  ...
  // AKE: Verify cookie timestamp (Toxcore has a time out of 15 seconds for
  // cookie packets.)
 ...
 *
 * AKE: Peer B (receiver) receives a cookie request from another peer (e.g. Peer
 * A) AKE: Peer creates a cookie & the cookie response packet (using echoID_A
 * from Initiator) AKE NEW: same function as old handshake
 *
 * return -1 on failure.
 * return COOKIE_RESPONSE_LENGTH on success.
 */
static int create_cookie_response(const Net_Crypto *c, uint8_t *packet,
  ...
  /*
   * AKE NEW:########################################################
   * Here I would know the public key of the peer and could initialize the
   * handshake! But I don't have access to "conn" and conn->handshake Also the
   * peer shouldn't allocate any memory during the cookie phase (according to
   * spec)
   * -> no temporary handshake variable
   */

  // AKE copy the initiators/Peer A DHT public key to the (plain) cookie data
  memcpy(cookie_plain + CRYPTO_PUBLIC_KEY_SIZE, dht_public_key,
         CRYPTO_PUBLIC_KEY_SIZE);
  uint8_t plain[COOKIE_LENGTH + sizeof(uint64_t)];

  if (create_cookie(c->log, c->mono_time, plain, cookie_plain,
                    c->secret_symmetric_key) != 0) {
    return -1;
  }

  // AKE: this should be the line copying the echoID_A to the plaintext
  memcpy(plain + COOKIE_LENGTH, request_plain + COOKIE_DATA_LENGTH,
         sizeof(uint64_t));
  // AKE: set NET_PACKET_COOKIE_RESPONSE => for packet handling
  packet[0] = NET_PACKET_COOKIE_RESPONSE;
  // AKE: Nonce used to encrypt cookie response packet
  random_nonce(packet + 1);
  int len = encrypt_data_symmetric(shared_key, packet + 1, plain, sizeof(plain),
 ...
 * Put what was in the request in request_plain (must be of size
 * COOKIE_REQUEST_PLAIN_LENGTH) Put the key used to decrypt the request into
 * shared_key (of size CRYPTO_SHARED_KEY_SIZE) for use in the response.
 *
 * AKE: Peer B (receiver) handling a cookie request, retrieve plaintext from
 * cookie request (echoID_A) AKE NEW: same function as old handshake
 *
 * return -1 on failure.
 * return 0 on success.
 */
static int handle_cookie_request(const Net_Crypto *c, uint8_t *request_plain,
  ...
  int len = decrypt_data_symmetric(
      shared_key, packet + 1 + CRYPTO_PUBLIC_KEY_SIZE,
      packet + 1 + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_NONCE_SIZE,
      COOKIE_REQUEST_PLAIN_LENGTH + CRYPTO_MAC_SIZE, request_plain);
 ...
 */
static int udp_handle_cookie_request(void *object, IP_Port source,
                                     const uint8_t *packet, uint16_t length,
                                     void *userdata) {
  ...
  if (handle_cookie_request(c, request_plain, shared_key, dht_public_key,
                            packet, length) != 0) {
    return 1;
  }
  ...
  if (create_cookie_response(c, data, request_plain, shared_key,
                             dht_public_key) != sizeof(data)) {
    return 1;
  }

  if ((uint32_t)sendpacket(dht_get_net(c->dht), source, data, sizeof(data)) !=
      sizeof(data)) {
    return 1;
  }
 ...
 * Triggered by receiving of NET_PACKET_COOKIE_REQUEST packet
 */
static int tcp_handle_cookie_request(Net_Crypto *c, int connections_number,
                                     const uint8_t *packet, uint16_t length) {
  uint8_t request_plain[COOKIE_REQUEST_PLAIN_LENGTH];
  uint8_t shared_key[CRYPTO_SHARED_KEY_SIZE];
  uint8_t dht_public_key[CRYPTO_PUBLIC_KEY_SIZE];
  ...
  int ret = send_packet_tcp_connection(c->tcp_c, connections_number, data,
                                       sizeof(data));
  return ret;
}
 ...
 * Triggered by receiving of NET_PACKET_COOKIE_REQUEST packet
 */
static int tcp_oob_handle_cookie_request(const Net_Crypto *c,
  ...
  if (handle_cookie_request(c, request_plain, shared_key, dht_public_key_temp,
                            packet, length) != 0) {
    return -1;
  }
  ...
  int ret = tcp_send_oob_packet(c->tcp_c, tcp_connections_number,
                                dht_public_key, data, sizeof(data));
  return ret;
}
 ...
 *
 * AKE: Peer A/initiator receives a cookie response packet after sending a
 * cookie request packet AKE: Retrieve cookie from the other peer and verify
 * echoID AKE NEW: same funtion as in old handshake
 *
 *
 * return -1 on failure.
 * return COOKIE_LENGTH on success.
 */
static int handle_cookie_response(const Logger *log, uint8_t *cookie,
                                  uint64_t *number, const uint8_t *packet,
                                  uint16_t length, const uint8_t *shared_key) {
  ...
  const int len = decrypt_data_symmetric(
      shared_key, packet + 1, packet + 1 + CRYPTO_NONCE_SIZE,
...
//#define HANDSHAKE_PACKET_LENGTH (1 + COOKIE_LENGTH + CRYPTO_NONCE_SIZE +
//CRYPTO_NONCE_SIZE + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_SHA512_SIZE +
//COOKIE_LENGTH + CRYPTO_MAC_SIZE) AKE NEW: first one with base nonce, second
// one without base nonce #define HANDSHAKE_PACKET_LENGTH_RESPONDER (1 +
//COOKIE_LENGTH + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_NONCE_SIZE +
//CRYPTO_SHA512_SIZE + COOKIE_LENGTH + CRYPTO_MAC_SIZE)
#define HANDSHAKE_PACKET_LENGTH_RESPONDER                                      \
  (1 + COOKIE_LENGTH + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_SHA512_SIZE +           \
   COOKIE_LENGTH + CRYPTO_MAC_SIZE)
// AKE NEW: Initiator also sends ENCRYPTED static public key with MAC (first one
// with, second one without base nonce) #define HANDSHAKE_PACKET_LENGTH_INITIATOR
//(1 + COOKIE_LENGTH + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_PUBLIC_KEY_SIZE +
//CRYPTO_MAC_SIZE + CRYPTO_NONCE_SIZE + CRYPTO_SHA512_SIZE + COOKIE_LENGTH +
//CRYPTO_MAC_SIZE)
#define HANDSHAKE_PACKET_LENGTH_INITIATOR                                      \
  (1 + COOKIE_LENGTH + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_PUBLIC_KEY_SIZE +       \
   CRYPTO_MAC_SIZE + CRYPTO_SHA512_SIZE + COOKIE_LENGTH + CRYPTO_MAC_SIZE)
 ...
 * AKE NEW: function adapted, different handshake packets for INITIATOR and
 * RESPONDER
 *
 * return -1 on failure.
 * return HANDSHAKE_PACKET_LENGTH on success.
 */
static int create_crypto_handshake(const Net_Crypto *c, uint8_t *packet,
  ...
  if (role == NOISE_ROLE_RESPONDER) {
    ...
    /* Handshake packet structure
     ...
     ~~[24 bytes base nonce]~~ => NO base Nonce -> handled by Noise/XSalsa20 not
     possible [64 bytes sha512 hash of the entire Cookie sitting outside the
     encrypted part] [112 bytes Other Cookie (used by the other to respond to
    ...
    // AKE NEW: ephemeral public key of the RESPONDER + encrypted payload + MAC
    // of encrypted payload (with and without base nonce)
    //		uint8_t noise_message_buf[CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_NONCE_SIZE +
    //CRYPTO_SHA512_SIZE + COOKIE_LENGTH + CRYPTO_MAC_SIZE];
    uint8_t noise_message_buf[CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_SHA512_SIZE +
                              COOKIE_LENGTH + CRYPTO_MAC_SIZE];
    // AKE NEW: Application chosen payload of Noise message
    NoiseBuffer noise_payload;
    // AKE NEW: Payload contains NO base nonce, but sha512 hash of cookie
    // (Initiator), and other cookie (Responder)
    //		uint8_t noise_payload_buf[CRYPTO_NONCE_SIZE + CRYPTO_SHA512_SIZE +
    //COOKIE_LENGTH ];
    ...
    //		crypto_sha512(noise_payload_buf + CRYPTO_NONCE_SIZE, cookie,
    //COOKIE_LENGTH);
    crypto_sha512(noise_payload_buf, cookie, COOKIE_LENGTH);
    uint8_t othercookie_plain[COOKIE_DATA_LENGTH];
    // AKE: The cookie contains the static and DHT pubkey of the peer! not from
    // the sender!
    memcpy(othercookie_plain, peer_real_pk, CRYPTO_PUBLIC_KEY_SIZE);
    // AKE: The cookie contains the static and DHT pubkey of the peer! not from
    // the sender!
    memcpy(othercookie_plain + CRYPTO_PUBLIC_KEY_SIZE, peer_dht_pubkey,
           CRYPTO_PUBLIC_KEY_SIZE);
    ...
    if (create_cookie(c->log, c->mono_time,
                      noise_payload_buf + CRYPTO_SHA512_SIZE, othercookie_plain,
                      c->secret_symmetric_key) != 0) {
      return -1;
    }

    // AKE NEW: Format the message payload into "payloadbuf".
    noise_buffer_set_input(noise_payload, noise_payload_buf,
                           sizeof(noise_payload_buf));
    // AKE NEW: Points to the message buffer to be populated with handshake
    // details and the message payload.
    noise_buffer_set_output(noise_message, noise_message_buf,
                            sizeof(noise_message_buf));
    // AKE NEW: Computes Noise message B pattern "e, ee, se"
    int err = noise_handshakestate_write_message(handshake, &noise_message,
                                                 &noise_payload);
    ...
    // AKE NEW: write packet kind, cookie and noise message incl. payload to
    // packet
    packet[0] = NET_PACKET_CRYPTO_HS;
    memcpy(packet + 1, cookie, COOKIE_LENGTH);
    memcpy(packet + 1 + COOKIE_LENGTH, noise_message_buf,
           sizeof(noise_message_buf));
    ...
    // AKE NEW: Noise message + Application chosen payload
    NoiseBuffer noise_message;
    // AKE NEW: ephemeral pubkey INITIATOR + static pubkey INITIATOR + MAC
    // static pubkey + encrypted payload (without base nonce) + MAC of encrypted
    // payload
    uint8_t noise_message_buf[CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_PUBLIC_KEY_SIZE +
                              CRYPTO_MAC_SIZE + CRYPTO_SHA512_SIZE +
                              COOKIE_LENGTH + CRYPTO_MAC_SIZE];
    // AKE NEW: Application chosen payload of Noise message
    NoiseBuffer noise_payload;
    // AKE NEW: Payload contains NO base nonce, but sha512 hash of cookie
    // (RESPONDER), and other cookie (INITIATOR)
    uint8_t noise_payload_buf[CRYPTO_SHA512_SIZE + COOKIE_LENGTH];
    // AKE NEW: Add Nonce to payload => NOT necessary
    //		memcpy(noise_payload_buf, nonce, CRYPTO_NONCE_SIZE);
    crypto_sha512(noise_payload_buf, cookie, COOKIE_LENGTH);
    ...
    noise_buffer_set_input(noise_payload, noise_payload_buf,
                           sizeof(noise_payload_buf));
    // AKE NW: Points to the message buffer to be populated with handshake
    // details and the message payload.
    noise_buffer_set_output(noise_message, noise_message_buf,
                            sizeof(noise_message_buf));
  ...
  //CRYPTO_SHA512_SIZE + COOKIE_LENGTH ]; 	memcpy(plain, nonce,
  //CRYPTO_NONCE_SIZE); 	memcpy(plain + CRYPTO_NONCE_SIZE, session_pk,
  //CRYPTO_PUBLIC_KEY_SIZE);
  //	// AKE: This is the cookie from Peer B/receiver which is sent in plain
  //(see handshake packet in spec) 	crypto_sha512(plain + CRYPTO_NONCE_SIZE +
  //CRYPTO_PUBLIC_KEY_SIZE, cookie, COOKIE_LENGTH); 	uint8_t
  //cookie_plain[COOKIE_DATA_LENGTH ]; 	memcpy(cookie_plain, peer_real_pk,
  //CRYPTO_PUBLIC_KEY_SIZE); 	memcpy(cookie_plain + CRYPTO_PUBLIC_KEY_SIZE,
  //peer_dht_pubkey, CRYPTO_PUBLIC_KEY_SIZE);
  //
  //	// AKE: This is to create the OTHER cookie from Peer A/initiator
  //	// AKE: Also Peer B sends a cookie again in the handshake packet
  //	if (create_cookie(c->log, c->mono_time, plain + CRYPTO_NONCE_SIZE +
  //CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_SHA512_SIZE, 			cookie_plain,
  //c->secret_symmetric_key) != 0) { 		return -1;
  //	}
  //
  //	random_nonce(packet + 1 + COOKIE_LENGTH);
  //	// AKE: handshake packet is using peers real STATIC public and self
  //STATIC secret key for encryption 	int len = encrypt_data(peer_real_pk,
  //c->self_secret_key, packet + 1 + COOKIE_LENGTH, plain, sizeof(plain), 			packet
  //+ 1 + COOKIE_LENGTH + CRYPTO_NONCE_SIZE);
  //
  //	//AKE NEW: I think in the Noise handshake this is not necessary
  //	if (len != HANDSHAKE_PACKET_LENGTH - (1 + COOKIE_LENGTH +
  //CRYPTO_NONCE_SIZE)) { 		return -1;
  //	}
  //
  //	packet[0] = NET_PACKET_CRYPTO_HS;
  //	memcpy(packet + 1, cookie, COOKIE_LENGTH);
 ...
 * AKE NEW: This function is called if Noise handshake state is
 * NOISE_ACTION_READ_MESSAGE AKE NEW: function adapted because of different
 * handshake packets for INITIATOR and RESPONDER
 *
 * return -1 on failure.
 * return 0 on success.
 */
static int handle_crypto_handshake(const Net_Crypto *c, uint8_t *peer_real_pk,
                                   uint8_t *dht_public_key, uint8_t *cookie,
                                   const uint8_t *packet, uint16_t length,
  ...
  /*
   * AKE: COOKIES ARE ENCRYPTED WITH AN ADDITIONAL SECRET SYMMETRIC KEY WHICH IS
   * ONLY KNOWN TO THE PEER HIMSELF! AKE: The peer opens his cookie to verify
   * it's really from him! AKE: cookie_plain contains static public key and DHT
   * public key of THIS peer
   */
  if (open_cookie(c->log, c->mono_time, cookie_plain, packet + 1,
                  c->secret_symmetric_key) != 0) {
    return -1;
  }
  ...
  if (role == NOISE_ROLE_RESPONDER) {
    // AKE NEW: Noise message + Application chosen payload
    NoiseBuffer noise_message;
    // AKE NEW: ephemeral pubkey + encrypted INITIATOR static pubkey + MAC
    // static pubkey + encrypted payload (without base nonce) + MAC of encrypted
    // payload
    uint8_t noise_message_buf[CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_PUBLIC_KEY_SIZE +
                              CRYPTO_MAC_SIZE + CRYPTO_SHA512_SIZE +
    ...
    memcpy(noise_message_buf, packet + 1 + COOKIE_LENGTH,
           HANDSHAKE_PACKET_LENGTH_INITIATOR - 1 - COOKIE_LENGTH);
    // AKE NEW: Application chosen payload of Noise message
    NoiseBuffer noise_payload;
    // AKE NEW: Payload contains NO base nonce, but sha512 hash of cookie
    // (RESPONDER), and other cookie (INITIATOR)
    ...
    // AKE NEW: Points to the message buffer to be populated with handshake
    // details and the message payload.
    noise_buffer_set_input(noise_message, noise_message_buf,
                           sizeof(noise_message_buf));
    // AKE NEW: Computes Noise message A pattern "e, es, s, ss" on RESPONDER
    // side
    int err = noise_handshakestate_read_message(handshake, &noise_message,
                                                &noise_payload);
    if (err != NOISE_ERROR_NONE) {
    ...
    if (crypto_memcmp(cookie_hash, noise_payload.data, CRYPTO_SHA512_SIZE) !=
        0) {
      return -1;
    }
    // AKE NEW: memcpy peer_real_pk from cookie_plain + dht_public_key
    memcpy(peer_real_pk, cookie_plain, CRYPTO_PUBLIC_KEY_SIZE);
    memcpy(dht_public_key, cookie_plain + CRYPTO_PUBLIC_KEY_SIZE,
           CRYPTO_PUBLIC_KEY_SIZE);
  ...
  // packet
    ...
    // AKE NEW: Noise message + Application chosen payload
    NoiseBuffer noise_message;
    // AKE NEW: ephemeral pubkey RESPONDER + encrypted payload (without base
    // nonce) + MAC of encrypted payload
    uint8_t noise_message_buf[CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_SHA512_SIZE +
                              COOKIE_LENGTH + CRYPTO_MAC_SIZE];
    // AKE NEW: Copy the data from packet to the noise_message_buf
    memcpy(noise_message_buf, packet + 1 + COOKIE_LENGTH,
           HANDSHAKE_PACKET_LENGTH_RESPONDER - 1 - COOKIE_LENGTH);
    // AKE NEW: Application chosen payload of Noise message
    NoiseBuffer noise_payload;
    // AKE NEW: Payload contains NO base nonce, sha512 hash of cookie, and other
    // cookie
    uint8_t noise_payload_buf[CRYPTO_SHA512_SIZE + COOKIE_LENGTH];
    // AKE NEW: Format the message payload into "payloadbuf".
    noise_buffer_set_input(noise_payload, noise_payload_buf,
  ...
  //	uint8_t plain[CRYPTO_NONCE_SIZE + CRYPTO_PUBLIC_KEY_SIZE +
  //CRYPTO_SHA512_SIZE + COOKIE_LENGTH ]; 	int len = decrypt_data(cookie_plain,
  //c->self_secret_key, packet + 1 + COOKIE_LENGTH, 			packet + 1 + COOKIE_LENGTH +
  //CRYPTO_NONCE_SIZE, 			HANDSHAKE_PACKET_LENGTH - (1 + COOKIE_LENGTH +
  //CRYPTO_NONCE_SIZE), plain);
  //
  //	if (len != sizeof(plain)) {
  //		return -1;
  //	}
  //
  //	if (crypto_memcmp(cookie_hash, plain + CRYPTO_NONCE_SIZE +
  //CRYPTO_PUBLIC_KEY_SIZE, 	CRYPTO_SHA512_SIZE) != 0) { 		return -1;
  //	}
  //
  ...
  //CRYPTO_SHA512_SIZE, COOKIE_LENGTH); 	memcpy(peer_real_pk, cookie_plain,
  //CRYPTO_PUBLIC_KEY_SIZE); 	memcpy(dht_public_key, cookie_plain +
...
static Crypto_Connection *get_crypto_connection(const Net_Crypto *c,
                                                int crypt_connection_id) {
 ...
 * return 0 on success.
 */
static int add_ip_port_connection(Net_Crypto *c, int crypt_connection_id,
                                  IP_Port ip_port) {
  ...
  if (net_family_is_ipv4(ip_port.ip.family)) {
    if (!ipport_equal(&ip_port, &conn->ip_portv4) &&
        !ip_is_lan(conn->ip_portv4.ip)) {
      if (!bs_list_add(&c->ip_port_list, (uint8_t *)&ip_port,
                       crypt_connection_id)) {
        return -1;
      }

      bs_list_remove(&c->ip_port_list, (uint8_t *)&conn->ip_portv4,
                     crypt_connection_id);
    ...
    if (!ipport_equal(&ip_port, &conn->ip_portv6)) {
      ...
      bs_list_remove(&c->ip_port_list, (uint8_t *)&conn->ip_portv6,
                     crypt_connection_id);
 ...
 */
static IP_Port return_ip_port_connection(Net_Crypto *c,
                                         int crypt_connection_id) {
...
/*
 * AKE NEW: Initializes a NoiseHandshakeState with provided secret key and peer
 * public key (if INITIATOR) AKE NEW: Noise prologue used in Tox is
 * CRYPTO_NOISE_PROTOCOL_NAME AKE NEW: don't need *prologue and prologue_len as
 * parameters (since it's fixed in Tox)
 *
 * return -1 on failure
 * return 0 on success
 */
static int initialize_handshake
    //(NoiseHandshakeState *handshake, const Net_Crypto *c, int
    //crypt_connection_id)
    (NoiseHandshakeState *handshake, const uint8_t *self_secret_key,
     const uint8_t *peer_public_key) {
  printf("ENTERING: initialize_handshake()");
  // AKE NEW: don't think this is necessary, just pass conn stuff diretly
  // Crypto_Connection *conn = get_crypto_connection(c, crypt_connection_id);

  // AKE NEW: DHState objects are used to store the keypairs for the local party
  // or the public keys for remote parties.
  NoiseDHState *dh;
  size_t key_len = 0;
  int err;

  // AKE NEW: set prologue to CRYPTO_NOISE_PROTOCOL_NAME
  err = noise_handshakestate_set_prologue(handshake, CRYPTO_NOISE_PROTOCOL_NAME,
                                          sizeof(CRYPTO_NOISE_PROTOCOL_NAME));
  if (err != NOISE_ERROR_NONE) {
  ...
  // AKE NEW: IK pattern: if role = INITIATOR => peer_public_key is also
  // necessary
  int role = noise_handshakestate_get_role(handshake);
  if (role == NOISE_ROLE_INITIATOR) {
    if (peer_public_key) {
      ...
      // AKE NEW: set remote peer "key" to Crypto_Connection *conn:
      // conn->public_key err = noise_dhstate_set_public_key(dh,
      // conn->public_key, key_len);
 ...
 * return 0 on success.
 */
static int send_packet_to(Net_Crypto *c, int crypt_connection_id,
  ...
  // TODO(irungentoo): on bad networks, direct connections might not last
  // indefinitely.
  if (!net_family_is_unspec(ip_port.ip.family)) {
    ...
    if (direct_connected) {
      if ((uint32_t)sendpacket(dht_get_net(c->dht), ip_port, data, length) ==
          length) {
    ...
    // TODO(irungentoo): a better way of sending packets directly to confirm the
    // others ip.
    const uint64_t current_time = mono_time_get(c->mono_time);

    if ((((UDP_DIRECT_TIMEOUT / 2) + conn->direct_send_attempt_time) >
             current_time &&
         length < 96) ||
        data[0] == NET_PACKET_COOKIE_REQUEST ||
        data[0] == NET_PACKET_CRYPTO_HS) {
      if ((uint32_t)sendpacket(dht_get_net(c->dht), ip_port, data, length) ==
          length) {
  ...
  int ret = send_packet_tcp_connection(c->tcp_c, conn->connection_number_tcp,
                                       data, length);
 ...
 * return 0 on success.
 */
static int add_data_to_buffer(const Logger *log, Packets_Array *array,
                              uint32_t number, const Packet_Data *data) {
 ...
 */
static int get_data_pointer(const Logger *log, const Packets_Array *array,
                            Packet_Data **data, uint32_t number) {
  const uint32_t num_spots = num_packets_array(array);

  if (array->buffer_end - number > num_spots ||
      number - array->buffer_start >= num_spots) {
    return -1;
  }
 ...
 */
static int64_t add_data_end_of_buffer(const Logger *log, Packets_Array *array,
 ...
 * return packet number on success.
 */
static int64_t read_data_beg_buffer(const Logger *log, Packets_Array *array,
                                    Packet_Data *data) {
 ...
 * return 0 on success
 */
static int clear_buffer_until(const Logger *log, Packets_Array *array,
                              uint32_t number) {
  const uint32_t num_spots = num_packets_array(array);
 ...
 * return 0 on success.
 */
static int set_buffer_end(const Logger *log, Packets_Array *array,
                          uint32_t number) {
  if (number - array->buffer_start > CRYPTO_PACKET_BUFFER_SIZE) {
    return -1;
  }
 ...
 */
static int generate_request_packet(const Logger *log, uint8_t *data,
                                   uint16_t length,
                                   const Packets_Array *recv_array) {
  if (length == 0) {
    return -1;
  }
 ...
 */
static int handle_request_packet(Mono_Time *mono_time, const Logger *log,
                                 Packets_Array *send_array, const uint8_t *data,
                                 uint16_t length, uint64_t *latest_send_time,
                                 uint64_t rtt_time) {
  if (length == 0) {
    return -1;
  }
  ...
  for (uint32_t i = send_array->buffer_start; i != send_array->buffer_end;
       ++i) {
    if (length == 0) {
      break;
    }
...
#define MAX_DATA_DATA_PACKET_SIZE                                              \
  (MAX_CRYPTO_PACKET_SIZE - (1 + sizeof(uint16_t) + CRYPTO_MAC_SIZE))

/* Creates and sends a data packet to the peer using the fastest route.
 *
 * AKE NEW: function adapted because we need to use ChaCha20 via
 * noise_cipherstate_encrypt() because there is (currently) no possibility to
 * retrieve the symmetric key from a NoiseCipherState.
 *
 * return -1 on failure.
 * return 0 on success.
 */
static int send_data_packet(Net_Crypto *c, int crypt_connection_id,
                            const uint8_t *data, uint16_t length) {
  // AKE NEW: + sizeof(uint16_t) + => last 2 bytes nonce used for encryption ->
  // removed const uint16_t max_length = MAX_CRYPTO_PACKET_SIZE - (1 +
  // sizeof(uint16_t) + CRYPTO_MAC_SIZE);
  ...
  pthread_mutex_lock(conn->mutex);
  // AKE NEW: removed sizeof(uint16_t) => last 2 bytes of base nonce => NO base
  // nonce VLA(uint8_t, packet, 1 + sizeof(uint16_t) + length + CRYPTO_MAC_SIZE);
  VLA(uint8_t, packet, 1 + length + CRYPTO_MAC_SIZE);
  packet[0] = NET_PACKET_CRYPTO_DATA;
  // AKE NEW: copies the last 2 bytes of the nonce => NO base nonce
  //	memcpy(packet + 1, conn->sent_nonce + (CRYPTO_NONCE_SIZE -
  //sizeof(uint16_t)), sizeof(uint16_t)); AKE NEW: can't use conn->send_cipher
  // directly because it's not only the key
  //	const int len = encrypt_data_symmetric(conn->shared_key,
  //conn->sent_nonce, data, length, packet + 1 + sizeof(uint16_t));
  ...
  noise_buffer_set_inout(noise_message, noise_message_buf, length,
                         sizeof(noise_message_buf));
  ...
  if (err != NOISE_ERROR_NONE) {
    if (err == NOISE_ERROR_INVALID_NONCE) {
       ...
       * for encryption would be catastrophic. Implementations must carefully
       * follow the rules for nonces. Nonces are not allowed to wrap back to
       * zero due to integer overflow, and the maximum nonce value is reserved.
       * This means parties are not allowed to send more than 2^64-1 transport
       * messages." "If EncryptWithAd() or DecryptWithAd() signal an error due
       * to nonce exhaustion, then the application must delete the CipherState
       * and terminate the session."
       * => noise_cipherstate_decrypt() -> NOISE_ERROR_INVALID_NONCE if the
       * nonce previously overflowed.
       */
      // AKE NEW TODO: connection_kill() / crypto_kill() /
      // wipe_crypto_connection()? => wipe not available here
  ...
  /*
   * AKE NEW: own Noise encryption function like encrypt_data_symmetric() ->
   * also there is padding?
   * => I think not necessary
   * Padding:
   * <@iphy> tb`: WARNING: Messages in the C NaCl API are 0-padded versions of
   * messages in the C++ NaCl API. Specifically: The caller must ensure, before
   * calling the C NaCl crypto_box function, that the first crypto_box_ZEROBYTES
   * bytes of the message m are all 0. Typical higher-level applications will
   * work with the remaining bytes of the message; note, however, that mlen
...
/* Creates and sends a data packet with buffer_start and num to the peer using
 * the fastest route.
 *
 * return -1 on failure.
 * return 0 on success.
 */
static int send_data_packet_helper(Net_Crypto *c, int crypt_connection_id,
                                   uint32_t buffer_start, uint32_t num,
                                   const uint8_t *data, uint16_t length) {
  ...
  uint16_t padding_length =
      (MAX_CRYPTO_DATA_SIZE - length) % CRYPTO_MAX_PADDING;
  VLA(uint8_t, packet,
      sizeof(uint32_t) + sizeof(uint32_t) + padding_length + length);
...
static int reset_max_speed_reached(Net_Crypto *c, int crypt_connection_id) {
  ...
  if (conn->maximum_speed_reached) {
    ...
    const int ret =
        get_data_pointer(c->log, &conn->send_array, &dt, packet_num);

    if (ret == 1 && dt->sent_time == 0) {
      if (send_data_packet_helper(c, crypt_connection_id,
                                  conn->recv_array.buffer_start, packet_num,
                                  dt->data, dt->length) != 0) {
        return -1;
      }
 ...
 */
static int64_t send_lossless_packet(Net_Crypto *c, int crypt_connection_id,
                                    const uint8_t *data, uint16_t length,
  ...
  if (send_data_packet_helper(c, crypt_connection_id,
                              conn->recv_array.buffer_start, packet_num, data,
                              length) == 0) {
...
static uint16_t get_nonce_uint16(const uint8_t *nonce) {
  uint16_t num;
  memcpy(&num, nonce + (CRYPTO_NONCE_SIZE - sizeof(uint16_t)),
         sizeof(uint16_t));
 ...
 * AKE NEW: function adapted because we need to use ChaCha20 via
 * noise_cipherstate_encrypt() because there is (currently) no possibility to
 * retrieve the symmetric key from a NoiseCipherState. AKE NEW: Base nonce not
 * necessary because nonce is included in NoiseCipherState
 *
 * return -1 on failure.
 * return length of data on success.
 */
static int handle_data_packet(const Net_Crypto *c, int crypt_connection_id,
                              uint8_t *data, const uint8_t *packet,
                              uint16_t length) {
  // AKE NEW: removed nonce from packet
  // const uint16_t crypto_packet_overhead = 1 + sizeof(uint16_t) +
  // CRYPTO_MAC_SIZE;
  ...
  // => NOT POSSIBLE
  //	int len = decrypt_data_symmetric(conn->shared_key, nonce, packet + 1 +
  //sizeof(uint16_t), 			length - (1 + sizeof(uint16_t)), data);
...
static int send_request_packet(Net_Crypto *c, int crypt_connection_id) {
  ...
  uint8_t data[MAX_CRYPTO_DATA_SIZE];
  int len =
      generate_request_packet(c->log, data, sizeof(data), &conn->recv_array);
  ...
  return send_data_packet_helper(c, crypt_connection_id,
                                 conn->recv_array.buffer_start,
                                 conn->send_array.buffer_end, data, len);
}
 ...
 */
static int send_requested_packets(Net_Crypto *c, int crypt_connection_id,
                                  uint32_t max_num) {
  if (max_num == 0) {
    return -1;
  }
 ...
 * return 0 on success.
 */
static int new_temp_packet(const Net_Crypto *c, int crypt_connection_id,
                           const uint8_t *packet, uint16_t length) {
...
static int send_temp_packet(Net_Crypto *c, int crypt_connection_id) {
  ...
  if (send_packet_to(c, crypt_connection_id, conn->temp_packet,
                     conn->temp_packet_length) != 0) {
    return -1;
  }
 ...
 *
 * AKE: Peer A/Initiator sends handshake packet after receiving of cookie
 * response. Function also used by Peer B/receiver AKE NEW: function adapted
 * because handshake packet lengths are now different for INITIATOR and
 * RESPONDER
 *
 * return -1 on failure.
 * return 0 on success.
 */
static int create_send_handshake(Net_Crypto *c, int crypt_connection_id,
                                 const uint8_t *cookie,
                                 const uint8_t *dht_public_key) {
  ...
  if (role == NOISE_ROLE_INITIATOR) {
    ...
    if (create_crypto_handshake(c, handshake_packet, cookie, conn->public_key,
                                dht_public_key,
                                conn->handshake) != sizeof(handshake_packet)) {
      return -1;
    }
    if (new_temp_packet(c, crypt_connection_id, handshake_packet,
                        sizeof(handshake_packet)) != 0) {
      return -1;
    }
...
static int send_kill_packet(Net_Crypto *c, int crypt_connection_id) {
  ...
  return send_data_packet_helper(
      c, crypt_connection_id, conn->recv_array.buffer_start,
      conn->send_array.buffer_end, &kill_packet, sizeof(kill_packet));
}
...
static void connection_kill(Net_Crypto *c, int crypt_connection_id,
  ...
  if (conn->connection_status_callback) {
    conn->connection_status_callback(conn->connection_status_callback_object,
                                     conn->connection_status_callback_id, 0,
                                     userdata);
  }
 ...
 * return 0 on success.
 */
static int handle_data_packet_core(Net_Crypto *c, int crypt_connection_id,
                                   const uint8_t *packet, uint16_t length,
                                   bool udp, void *userdata) {
  fprintf(stderr, "ENTERING: handle_data_packet_core(); PACKET: %d\n",
          packet[0]);
  if (length > MAX_CRYPTO_PACKET_SIZE ||
      length <= CRYPTO_DATA_PACKET_MIN_SIZE) {
    return -1;
  }
  ...
  if (buffer_start != conn->send_array.buffer_start) {
    ...
    if (get_data_pointer(c->log, &conn->send_array, &packet_time,
                         conn->send_array.buffer_start) == 1) {
  ...
  if (conn->status == CRYPTO_CONN_NOT_CONFIRMED) {
    ...
    if (conn->connection_status_callback) {
      conn->connection_status_callback(conn->connection_status_callback_object,
                                       conn->connection_status_callback_id, 1,
                                       userdata);
    }
  }
  ...
  if (real_data[0] == PACKET_ID_REQUEST) {
    ...
    int requested =
        handle_request_packet(c->mono_time, c->log, &conn->send_array,
  ...
  } else if (real_data[0] >= PACKET_ID_RANGE_LOSSLESS_START &&
             real_data[0] <= PACKET_ID_RANGE_LOSSLESS_END) {
    Packet_Data dt = {0};
    ...
    if (conn->connection_lossy_data_callback) {
      conn->connection_lossy_data_callback(
          conn->connection_lossy_data_callback_object,
 ...
 *
 * AKE: handling of Cookie response packets, crypto handshake packets and crypto
 * data packets
 ...
 */
//
static int handle_packet_connection(Net_Crypto *c, int crypt_connection_id,
                                    const uint8_t *packet, uint16_t length,
                                    bool udp, void *userdata) {
  ...
  case NET_PACKET_COOKIE_RESPONSE: {
    ...
    if (handle_cookie_response(c->log, cookie, &number, packet, length,
                               conn->shared_key) != sizeof(cookie)) {
      return -1;
    }
    ...
    // AKE NEW: This function can only be called if new_crypto_connection() was
    // already called. No initialization necessary AKE NEW TODO: But maybe it's
    // better if we set the initiator here and not already in
    ...
    //		int err = noise_handshakestate_new_by_name(&conn->handshake,
    //CRYPTO_NOISE_PROTOCOL_NAME, NOISE_ROLE_INITIATOR); 		if (err !=
    //NOISE_ERROR_NONE) { 			noise_perror(CRYPTO_NOISE_PROTOCOL_NAME, err); 			return
    //-1;
    //		}
    //		//AKE NEW: initialize_handshake
    //		if (initialize_handshake(conn->handshake, c->self_secret_key,
    //conn->public_key) == -1) { 			noise_handshakestate_free(conn->handshake);
    //			return -1;
    //		}
    ...
    if (role == NOISE_ROLE_INITIATOR) {
      ...
      if (action == NOISE_ACTION_WRITE_MESSAGE) {
        // AKE: if cookie response was ok, create and send handshake packet to
        // Peer B/receiver
        if (create_send_handshake(c, crypt_connection_id, cookie,
                                  conn->dht_public_key) != 0) {
          return -1;
        }
      }
  ...
  // AKE NEW TODO: but maybe possible by differentiating role by using
  // CRYPTO_CONN_STATE? AKE NEW: THIS SEEMS TO BE THE ONLY POSSIBILITY TO HANDLE
  // A HANDSHAKE PACKET AS INITIATOR!
  case NET_PACKET_CRYPTO_HS: {
    fprintf(stderr,
            "ENTERING: handle_packet_connection(); PACKET: %d => "
            "NET_PACKET_CRYPTO_HS => CRYPTO CONN STATE: %d\n",
            packet[0], conn->status);
    // AKE: very likely, that "Peer B"/receiver (or Peer A/initiator) is in one
    // of these states
    if (conn->status != CRYPTO_CONN_COOKIE_REQUESTING &&
        conn->status != CRYPTO_CONN_HANDSHAKE_SENT
    ...
    if (role == NOISE_ROLE_INITIATOR) {
      ...
      if (action == NOISE_ACTION_READ_MESSAGE) {
        fprintf(
            stderr,
            "handle_packet_connection(); ACTION: NOISE_ACTION_READ_MESSAGE\n");
      ...
      if (noise_handshakestate_get_action(conn->handshake) !=
          NOISE_ACTION_SPLIT) {
        fprintf(stderr, "protocol handshake failed\n");
        noise_handshakestate_free(conn->handshake);
        conn->handshake = 0;
        // AKE NEW TODO: connection_kill() is possible here, or crypto_kill() or
        // wipe_crypto_connection() => wipe not available here
        //				wipe_crypto_connection(c,
        //crypt_connection_id);
        return -1;
      }
      /*AKE NEW: Split out the two CipherState objects for send and receive */
      else {
        int err = noise_handshakestate_split(
            conn->handshake, &conn->send_cipher, &conn->recv_cipher);
        if (err != NOISE_ERROR_NONE) {
          noise_perror("split to start data transfer", err);
          noise_handshakestate_free(conn->handshake);
          conn->handshake = 0;
          // AKE NEW TODO: crypto_kill() / wipe_crypto_connection() in this
          // case? connection_kill is possible here wipe not available here
          //					wipe_crypto_connection(c,
          //crypt_connection_id);
          return -1;
        }
    ...
    // AKE NEW TODO: adapt? Don't think this is working with INITIATOR/RESPONDER
    // roles AKE NEW TODO: try to handle with Noise action/state
    if (public_key_cmp(dht_public_key, conn->dht_public_key) == 0) {
      // AKE: Peer B (or Peer A, whoever receives the handshake packet)
      // calculation of shared session key
      // AKE NEW: handled by Noise
      // encrypt_precompute(conn->peersessionpublic_key,
      // conn->sessionsecret_key, conn->shared_key);

      /*
       * AKE: This seems to be an (error/not perfect handshake) case of Peer A
       * i.e. if Peer A receives handshake packet from Peer B, but has already
       * sent a cookie request to Peer B himself
       */
      if (conn->status == CRYPTO_CONN_COOKIE_REQUESTING) {
        if (create_send_handshake(c, crypt_connection_id, cookie,
                                  dht_public_key) != 0) {
      ...
      // AKE: change crypto conn state to CRYPTO_CONN_HANDSHAKE_SENT (cf.
      // accepted in spec)
      conn->status = CRYPTO_CONN_NOT_CONFIRMED;
    } else {
      if (conn->dht_pk_callback) {
        conn->dht_pk_callback(conn->dht_pk_callback_object,
                              conn->dht_pk_callback_number, dht_public_key,
                              userdata);
      }
    }
  ...
  case NET_PACKET_CRYPTO_DATA: {
    if (conn->status != CRYPTO_CONN_NOT_CONFIRMED &&
        conn->status != CRYPTO_CONN_ESTABLISHED) {
      fprintf(stderr,
              "ENTERING: handle_packet_connection(); PACKET: %d => ERROR => "
              "CRYPTO CONN STATE: %d\n",
              packet[0], conn->status);
      return -1;
    }
    return handle_data_packet_core(c, crypt_connection_id, packet, length, udp,
                                   userdata);
  }
...
static int realloc_cryptoconnection(Net_Crypto *c, uint32_t num) {
  ...
  Crypto_Connection *newcrypto_connections = (Crypto_Connection *)realloc(
      c->crypto_connections, num * sizeof(Crypto_Connection));
...
static int create_crypto_connection(Net_Crypto *c) {
  ...
  if (id != -1) {
    // Memsetting float/double to 0 is non-portable, so we explicitly set them
    // to 0
...
static int wipe_crypto_connection(Net_Crypto *c, int crypt_connection_id) {
  ...
  crypto_memzero(&c->crypto_connections[crypt_connection_id],
                 sizeof(Crypto_Connection));
 ...
 */
static int getcryptconnection_id(const Net_Crypto *c,
 ...
 */
static int crypto_connection_add_source(Net_Crypto *c, int crypt_connection_id,
                                        IP_Port source) {
  Crypto_Connection *conn = get_crypto_connection(c, crypt_connection_id);
  ...
  if (net_family_is_ipv4(source.ip.family) ||
      net_family_is_ipv6(source.ip.family)) {
  ...
  if (net_family_is_tcp_family(source.ip.family)) {
    if (add_tcp_number_relay_connection(c->tcp_c, conn->connection_number_tcp,
                                        source.ip.ip.v6.uint32[0]) == 0) {
      return 1;
    }
  }
 ...
 */
void new_connection_handler(Net_Crypto *c,
                            new_connection_cb *new_connection_callback,
                            void *object) {
...
/* Handle a handshake packet by someone who wants to initiate a new connection
 * with us. This calls the callback set by new_connection_handler() if the
 * handshake is ok.
 *
 * AKE: Peer B/receiver handles incoming handshake packet and sends a handshake
 * packet himself AKE NEW: function adapted for Noise handshake => RESPONDER
 * case AKE NEW: PROBLEM - I don't know the peers public key to initialize a
 ...
 * return 0 on success.
 */
static int handle_new_connection_handshake(Net_Crypto *c, IP_Port source,
                                           const uint8_t *data, uint16_t length,
                                           void *userdata) {
  fprintf(stderr, "ENTERING: handle_new_connection_handshake()\n");
  /*
   * AKE: This function seems to be called if Peer B/receiver doesn't have a
   * Crypto_Connection conn yet (with the peer it received the handshake packet
   * from)!
  ...
  // AKE NEW: add HandshakeState to New_Connection? Or a local variable here? =>
  // New_Connection for accept_crypto_connection! NoiseHandshakeState
  // *handshake_temp;
  ...
  int err = noise_handshakestate_new_by_name(
      &n_c.handshake_temp, CRYPTO_NOISE_PROTOCOL_NAME, NOISE_ROLE_RESPONDER);
  if (err != NOISE_ERROR_NONE) {
  ...
  // AKE NEW: initialize_handshake
  // AKE NEW: IMPORTANT n_c.publicy_key is empty/not defined here, it's set
  // handle_crypto_handshake()...
  if (initialize_handshake(n_c.handshake_temp, c->self_secret_key,
                           n_c.public_key) == -1) {
  ...
  if (role == NOISE_ROLE_RESPONDER) {
    ...
    if (err != NOISE_ERROR_NONE) {
      noise_perror("start handshake", err);
      // AKE NEW: kill / wipe crypto connection? => no crypto connection here
      // (is created afterwards)
      return -1;
    }
    int action = noise_handshakestate_get_action(n_c.handshake_temp);
    if (action == NOISE_ACTION_READ_MESSAGE) {
      // AKE: Handle the crypto handshake packet from Peer A/initiator (incl.
      // Other Cookie from A)
  ...
  if (crypt_connection_id != -1) {
    ...
    if (public_key_cmp(n_c.dht_public_key, conn->dht_public_key) != 0) {
      ...
      //			memcpy(conn->handshake, handshake_temp,
      //sizeof(handshake_temp));
      conn->handshake = n_c.handshake_temp;
      // AKE NEW: free handshake -> No:
      /*
       * An important note about the copying: It's a shallow copy, just like
       * with memcpy. That means if you have e.g. a structure containing
       * pointers, it's only the actual pointers that will be copied and not
       * what they point to, so after the copy you will have two pointers
      ...
      //			memcpy(conn->recv_nonce, n_c.recv_nonce,
      //CRYPTO_NONCE_SIZE);
      // AKE: Peer A/initiator session pk is copied into the crypto connection
      // from the new connection
      // AKE NEW: conn->peersessionpublic_key is NOT needed for new handshake
      //			memcpy(conn->peersessionpublic_key,
      //n_c.peersessionpublic_key, CRYPTO_PUBLIC_KEY_SIZE);
      // AKE: Peer B/receiver calculates shared session key from Peer
      // A/initiator public key and self session secret key AKE: Where is
      // conn->sessionsecret_key coming from? => conn already existed -> there
      // it is saved! Keys are generated in new_net_crypto()!
      // AKE NEW: handeled by noise
      //			encrypt_precompute(conn->peersessionpublic_key,
      //conn->sessionsecret_key, conn->shared_key);
      crypto_connection_add_source(c, crypt_connection_id, source);

      // AKE: Send handshake packet/message 2 Peer B/receiver -> Peer
      // A/initiator
      int role = noise_handshakestate_get_role(conn->handshake);
      if (role == NOISE_ROLE_RESPONDER) {
        int action = noise_handshakestate_get_action(conn->handshake);
        if (action == NOISE_ACTION_WRITE_MESSAGE) {
          // AKE: if cookie response was ok, create and send handshake packet to
          // Peer B/receiver
          if (create_send_handshake(c, crypt_connection_id, n_c.cookie,
                                    n_c.dht_public_key) != 0) {
            free(n_c.cookie);
            return -1;
          }
        }
        ...
        if (noise_handshakestate_get_action(conn->handshake) !=
          ...
          noise_handshakestate_free(conn->handshake);
          // AKE NEW: Only free one of the handshake objects, otherwise it's a
        ...
        /*AKE NEW: Split out the two CipherState objects for send and receive */
          ...
          fprintf(stderr,
                  "handle_new_connection_handshake(): NOISE SPLIT OK\n");
          /*AKE NEW: We no longer need the HandshakeState */
          noise_handshakestate_free(conn->handshake);
          // AKE NEW: Only free one of the handshake objects, otherwise it's a
          // double free/segfault
      ...
      // AKE: TODO in accept_crypto_connection this happens before this peer
      // calls create_send_handshake() -> shoudldn't this be also the case here?
      conn->status = CRYPTO_CONN_NOT_CONFIRMED;
      free(n_c.cookie);
      return 0;
    }
  ...
  // AKE: THIS SEEMS TO CALL accept_crypto_connection() => That's also why
  // encrypt_precompute() and create_send_handshake() are called twice
 ...
 * AKE: This function needs to be called by Peer B/receiver, otherwise he cannot
 * calculate his ephemeral keypair! (only if he also starts a handshake / calls
 * new_crypto_connection()) AKE: This is the related section in the spec: AKE:
 ...
 * AKE: This function doesn't call handle_crypto_handshake() because it's called
 * via handle_new_connection_handshake() and there also
 * handle_crypto_handshake() is called.
 *
 * return -1 on failure.
 * return connection id on success.
 */
int accept_crypto_connection(Net_Crypto *c, New_Connection *n_c) {
  ...
  pthread_mutex_lock(&c->tcp_mutex);
  const int connection_number_tcp =
      new_tcp_connection_to(c->tcp_c, n_c->dht_public_key, crypt_connection_id);
  pthread_mutex_unlock(&c->tcp_mutex);
  ...
  // AKE: the peer's session pubkey is copied into the crypto connection from
  // new connection
  // AKE NEW: NOT NECESSARY - handled by Noise
  // memcpy(conn->peersessionpublic_key, n_c->peersessionpublic_key,
  // CRYPTO_PUBLIC_KEY_SIZE);
  // AKE: Nonce is set here, therefore call to handle_new_connection_handshake()
  // only possible after call of this function!
  // AKE NEW: Nonce not necessary - handled by Noise
  //	random_nonce(conn->sent_nonce);
  // AKE: Message 2: generate new session public/private keypair -> peer
  // receiving the handshake packet
  // AKE NEW: not necessary
  // crypto_new_keypair(conn->sessionpublic_key, conn->sessionsecret_key);
  // AKE: Session Key Derivation by using session secret key and peer's session
  // public key AKE: peers session pk (handshake receiving peer) -> it's coming
  // from New_Connection n_c AKE: This shared_key should be hashed (HKDF) and
  // not just the raw ECDH result as secret used.
  // AKE NEW: not necessary (handled by Noise)
  // encrypt_precompute(conn->peersessionpublic_key, conn->sessionsecret_key,
  // conn->shared_key);

  // AKE: CRYPTO_CONN_NOT_CONFIRMED = ACCEPTED state of connection (need to send
  // handshake packet)
  conn->status = CRYPTO_CONN_NOT_CONFIRMED;
  ...
  if (role == NOISE_ROLE_RESPONDER) {
    ...
    if (noise_handshakestate_get_action(conn->handshake) !=
      ...
      // AKE NEW: handshake failed, free memory
      noise_handshakestate_free(conn->handshake);
      // AKE NEW: only free one of the handshake objects because otherwise it's
      // a double free/segfault
      //			noise_handshakestate_free(n_c->handshake_temp);
      conn->handshake = 0;
      //			n_c->handshake_temp = 0;
      // AKE NEW TODO: or wipe_crypto_connection() or kill?
    ...
    /*AKE NEW: Split out the two CipherState objects for send and receive */
      ...
      int err = noise_handshakestate_split(conn->handshake, &conn->send_cipher,
                                           &conn->recv_cipher);
      if (err != NOISE_ERROR_NONE) {
        noise_perror("split to start data transfer", err);
        noise_handshakestate_free(conn->handshake);
        // AKE NEW: only free one of the handshake objects because otherwise
        // it's a double free/segfault
        //				noise_handshakestate_free(n_c->handshake_temp);
        conn->handshake = 0;
        //				n_c->handshake_temp = 0;
        // AKE NEW TODO: or wipe_crypto_connection() or kill?
 ...
 * AKE: Perfect handshake: Peer A initiates handshake by creating and sending a
 * cookie request. AKE: Realistic handshake: also called by Peer B AKE NEW:
 * Function adapted for new handshake - INITIATOR CASE AKE NEW: This function is
 * likely to be called also by the other peer (at least if they are already
 * friends)!
 *
 * return -1 on failure.
 * return connection id on success.
...
int new_crypto_connection(Net_Crypto *c, const uint8_t *real_public_key,
  ...
  /* AKE: Sometimes toxcore might receive the DHT public key of the peer first
   ...
   * (DHT, TCP_connection) because this does happen.
   */
  const int connection_number_tcp =
      new_tcp_connection_to(c->tcp_c, dht_public_key, crypt_connection_id);
  pthread_mutex_unlock(&c->tcp_mutex);
  ...
  // AKE NEW: not necessary
  //	random_nonce(conn->sent_nonce);
  // AKE: Peer A handshake initiator _session_ public/private key pair
  // generation
  // AKE NEW: Not necessary - handled by Noise
  // crypto_new_keypair(conn->sessionpublic_key, conn->sessionsecret_key);
  ...
  int err = noise_handshakestate_new_by_name(
      &conn->handshake, CRYPTO_NOISE_PROTOCOL_NAME, NOISE_ROLE_INITIATOR);
  if (err != NOISE_ERROR_NONE) {
    noise_perror(CRYPTO_NOISE_PROTOCOL_NAME, err);
    return -1;
  }
  ...
  // AKE: Call: Create cookie request -> Initiator of handshake and send cookie
  // request AKE: Cookie request of initiator doesn't contain a cookie! Just to
  // request one from peer! AKE: conn->cookie_request_number is the echoID in
  // the cookie request packet!
  if (create_cookie_request(c, cookie_request, conn->dht_public_key,
                            conn->cookie_request_number,
                            conn->shared_key) != sizeof(cookie_request) ||
      new_temp_packet(c, crypt_connection_id, cookie_request,
                      sizeof(cookie_request)) != 0) {
    pthread_mutex_lock(&c->tcp_mutex);
    kill_tcp_connection_to(c->tcp_c, conn->connection_number_tcp);
    pthread_mutex_unlock(&c->tcp_mutex);
    wipe_crypto_connection(c, crypt_connection_id);
 ...
 *
 * Connected is 0 if we are not sure we are connected to that person, 1 if we
 * are sure.
 *
 * return -1 on failure.
 * return 0 on success.
...
int set_direct_ip_port(Net_Crypto *c, int crypt_connection_id, IP_Port ip_port,
  ...
  const uint64_t direct_lastrecv_time =
      connected ? mono_time_get(c->mono_time) : 0;
...
static int tcp_data_callback(void *object, int crypt_connection_id,
                             const uint8_t *data, uint16_t length,
                             void *userdata) {
  Net_Crypto *c = (Net_Crypto *)object;
  ...
  if (data[0] == NET_PACKET_COOKIE_REQUEST) {
    return tcp_handle_cookie_request(c, conn->connection_number_tcp, data,
                                     length);
  }
  ...
  pthread_mutex_unlock(&c->tcp_mutex);
  // AKE: One possiblity for Peer A/initator to handle a received cookie
  // response packet
  int ret = handle_packet_connection(c, crypt_connection_id, data, length, 0,
                                     userdata);
  pthread_mutex_lock(&c->tcp_mutex);
...
static int tcp_oob_callback(void *object, const uint8_t *public_key,
                            unsigned int tcp_connections_number,
                            const uint8_t *data, uint16_t length,
                            void *userdata) {
  Net_Crypto *c = (Net_Crypto *)object;
  ...
  if (data[0] == NET_PACKET_COOKIE_REQUEST) {
    return tcp_oob_handle_cookie_request(c, tcp_connections_number, public_key,
                                         data, length);
  }
  ...
  if (data[0] == NET_PACKET_CRYPTO_HS) {
    ...
    if (handle_new_connection_handshake(c, source, data, length, userdata) !=
        0) {
      return -1;
    }
 ...
 */
int add_tcp_relay_peer(Net_Crypto *c, int crypt_connection_id, IP_Port ip_port,
                       const uint8_t *public_key) {
  Crypto_Connection *conn = get_crypto_connection(c, crypt_connection_id);
  ...
  pthread_mutex_lock(&c->tcp_mutex);
  int ret = add_tcp_relay_connection(c->tcp_c, conn->connection_number_tcp,
                                     ip_port, public_key);
  pthread_mutex_unlock(&c->tcp_mutex);
  return ret;
}
...
/* Send an onion packet via the TCP relay corresponding to
...
int send_tcp_onion_request(Net_Crypto *c, unsigned int tcp_connections_number,
  ...
  pthread_mutex_lock(&c->tcp_mutex);
  int ret =
      tcp_send_onion_request(c->tcp_c, tcp_connections_number, data, length);
  pthread_mutex_unlock(&c->tcp_mutex);
 ...
 * NOTE that the family of the copied ip ports will be set to TCP_INET or
 * TCP_INET6.
 *
 * return number of relays copied to tcp_relays on success.
 * return 0 on failure.
 */
unsigned int copy_connected_tcp_relays(Net_Crypto *c, Node_format *tcp_relays,
                                       uint16_t num) {
  if (num == 0) {
    return 0;
  }
...
/* Set function to be called when connection with crypt_connection_id goes
 * connects/disconnects.
 *
 * The set function should return -1 on failure and 0 on success.
 * Note that if this function is set, the connection will clear itself on
 * disconnect. Object and id will be passed to this function untouched. status
 * is 1 if the connection is going online, 0 if it is going offline.
 *
 * return -1 on failure.
 * return 0 on success.
 */
...
/* Set function to be called when connection with crypt_connection_id receives a
 * data packet of length.
 *
 * The set function should return -1 on failure and 0 on success.
 ...
 * return 0 on success.
 */
int connection_lossy_data_handler(
    Net_Crypto *c, int crypt_connection_id,
    connection_lossy_data_cb *connection_lossy_data_callback, void *object,
    int id) {
  Crypto_Connection *conn = get_crypto_connection(c, crypt_connection_id);
...
/* Set the function for this friend that will be callbacked with object and
 * number if the friend sends us a different dht public key than we have
 * associated to him.
 *
 * If this function is called, the connection should be recreated with the new
 * public key.
 *
 ...
 *
 */
static int udp_handle_packet(void *object, IP_Port source,
                             const uint8_t *packet, uint16_t length,
                             void *userdata) {
  Net_Crypto *c = (Net_Crypto *)object;
  ...
  if (crypt_connection_id == -1) {
    ...
    if (handle_new_connection_handshake(c, source, packet, length, userdata) !=
        0) {
      return 1;
    }
  ...
  // AKE: One possiblity for Peer A/initator to handle a received cookie
  // response packet
  if (handle_packet_connection(c, crypt_connection_id, packet, length, 1,
                               userdata) != 0) {
    return 1;
  }
...
static void send_crypto_packets(Net_Crypto *c) {
  ...
  for (uint32_t i = 0; i < c->crypto_connections_length; ++i) {
    ...
    if ((CRYPTO_SEND_PACKET_INTERVAL + conn->temp_packet_sent_time) <
        temp_time) {
    ...
    if ((conn->status == CRYPTO_CONN_NOT_CONFIRMED ||
         conn->status == CRYPTO_CONN_ESTABLISHED) &&
        (CRYPTO_SEND_PACKET_INTERVAL + conn->last_request_packet_sent) <
            temp_time) {
    ...
    if (conn->status == CRYPTO_CONN_ESTABLISHED) {
      if (conn->packet_recv_rate > CRYPTO_PACKET_MIN_RATE) {
        double request_packet_interval =
            (REQUEST_PACKETS_COMPARE_CONSTANT /
             ((num_packets_array(&conn->recv_array) + 1.0) /
              (conn->packet_recv_rate + 1.0)));

        double request_packet_interval2 =
            ((CRYPTO_PACKET_MIN_RATE / conn->packet_recv_rate) *
             (double)CRYPTO_SEND_PACKET_INTERVAL) +
            (double)PACKET_COUNTER_AVERAGE_INTERVAL;
        ...
        if (temp_time - conn->last_request_packet_sent >
            (uint64_t)request_packet_interval) {
          if (send_request_packet(c, i) == 0) {
            conn->last_request_packet_sent = temp_time;
          }
        }
      ...
      if ((PACKET_COUNTER_AVERAGE_INTERVAL + conn->packet_counter_set) <
          temp_time) {
        ...
        unsigned int pos =
            conn->last_sendqueue_counter % CONGESTION_QUEUE_ARRAY_SIZE;
        conn->last_sendqueue_size[pos] = num_packets_array(&conn->send_array);

        long signed int sum = 0;
        sum =
            (long signed int)conn->last_sendqueue_size[pos] -
            (long signed int)conn
                ->last_sendqueue_size[(pos + 1) % CONGESTION_QUEUE_ARRAY_SIZE];

        unsigned int n_p_pos =
            conn->last_sendqueue_counter % CONGESTION_LAST_SENT_ARRAY_SIZE;
        ...
        conn->last_sendqueue_counter =
            (conn->last_sendqueue_counter + 1) %
            (CONGESTION_QUEUE_ARRAY_SIZE * CONGESTION_LAST_SENT_ARRAY_SIZE);

        bool direct_connected = 0;
        /* return value can be ignored since the `if` above ensures the
         * connection is established */
        crypto_connection_status(c, i, &direct_connected, nullptr);

        /* When switching from TCP to UDP, don't change the packet send rate for
         * CONGESTION_EVENT_TIMEOUT ms. */
        if (!(direct_connected &&
              conn->last_tcp_sent + CONGESTION_EVENT_TIMEOUT > temp_time)) {
          long signed int total_sent = 0, total_resent = 0;

          // TODO(irungentoo): use real delay
          unsigned int delay =
              (unsigned int)((conn->rtt_time /
                              PACKET_COUNTER_AVERAGE_INTERVAL) +
                             0.5);
          unsigned int packets_set_rem_array =
              (CONGESTION_LAST_SENT_ARRAY_SIZE - CONGESTION_QUEUE_ARRAY_SIZE);
          ...
          for (unsigned j = 0; j < CONGESTION_QUEUE_ARRAY_SIZE; ++j) {
            unsigned int ind = (j + (packets_set_rem_array - delay) + n_p_pos) %
          ...
          double min_speed_request =
              1000.0 *
              (((double)(total_sent + total_resent)) /
               ((double)(CONGESTION_QUEUE_ARRAY_SIZE)*PACKET_COUNTER_AVERAGE_INTERVAL));
          ...
          if (send_array_ratio > SEND_QUEUE_RATIO &&
              CRYPTO_MIN_QUEUE_LENGTH < npackets) {
            conn->packet_send_rate =
                min_speed * (1.0 / (send_array_ratio / SEND_QUEUE_RATIO));
          } else if (conn->last_congestion_event + CONGESTION_EVENT_TIMEOUT <
                     temp_time) {
      ...
      if (conn->last_packets_left_set == 0 ||
          conn->last_packets_left_requested_set == 0) {
        ...
        if (((uint64_t)((1000.0 / conn->packet_send_rate) + 0.5) +
             conn->last_packets_left_set) <= temp_time) {
          double n_packets =
              conn->packet_send_rate *
              (((double)(temp_time - conn->last_packets_left_set)) / 1000.0);
        ...
        if (((uint64_t)((1000.0 / conn->packet_send_rate_requested) + 0.5) +
             conn->last_packets_left_requested_set) <= temp_time) {
          double n_packets =
...
/* Return 1 if max speed was reached for this connection (no more data can be
 * physically through the pipe). Return 0 if it wasn't reached.
 */
 ...
 * return 0 if failure.
 */
uint32_t crypto_num_free_sendqueue_slots(const Net_Crypto *c,
                                         int crypt_connection_id) {
  Crypto_Connection *conn = get_crypto_connection(c, crypt_connection_id);
  ...
  uint32_t max_packets =
      CRYPTO_PACKET_BUFFER_SIZE - num_packets_array(&conn->send_array);
...
int64_t write_cryptpacket(Net_Crypto *c, int crypt_connection_id,
  ...
  if (data[0] < PACKET_ID_RANGE_LOSSLESS_START ||
      data[0] > PACKET_ID_RANGE_LOSSLESS_END) {
    return -1;
  }
 ...
 *
 * packet_number must be a valid packet number of a packet sent on this
 * connection.
 *
 * return -1 on failure.
 * return 0 on success.
 *
 * Note: The condition `buffer_end - buffer_start < packet_number -
 * buffer_start` is a trick which handles situations `buffer_end >=
 * buffer_start` and `buffer_end < buffer_start` (when buffer_end overflowed)
 * both correctly.
 *
 * It CANNOT be simplified to `packet_number < buffer_start`, as it will fail
 * when `buffer_end < buffer_start`.
 */
int cryptpacket_received(Net_Crypto *c, int crypt_connection_id,
 ...
 */
int send_lossy_cryptpacket(Net_Crypto *c, int crypt_connection_id,
                           const uint8_t *data, uint16_t length) {
  if (length == 0 || length > MAX_CRYPTO_DATA_SIZE) {
    return -1;
  }
  ...
  if (conn) {
    ...
    pthread_mutex_unlock(conn->mutex);
    ret = send_data_packet_helper(c, crypt_connection_id, buffer_start,
...
bool crypto_connection_status(const Net_Crypto *c, int crypt_connection_id,
  ...
  if (online_tcp_relays) {
    *online_tcp_relays = tcp_connection_to_online_tcp_relays(
        c->tcp_c, conn->connection_number_tcp);
  }
...
void save_keys(const Net_Crypto *c, uint8_t *keys) {
  ...
  memcpy(keys + CRYPTO_PUBLIC_KEY_SIZE, c->self_secret_key,
 ...
 */
// AKE: important function for every part of the handshake besides cookie
// request
Net_Crypto *new_net_crypto(const Logger *log, Mono_Time *mono_time, DHT *dht,
  ...
  temp->tcp_c =
      new_tcp_connections(mono_time, dht_get_self_secret_key(dht), proxy_info);
  ...
  // AKE: there is a new public/private key pair generated and set. Therefore
  // this should only be called once initially
  new_keys(temp);
  ...
  networking_registerhandler(dht_get_net(dht), NET_PACKET_COOKIE_REQUEST,
                             &udp_handle_cookie_request, temp);
...
void kill_net_crypto(Net_Crypto *c) {
  ...
  networking_registerhandler(dht_get_net(c->dht), NET_PACKET_COOKIE_REQUEST,
                             nullptr, nullptr);
  networking_registerhandler(dht_get_net(c->dht), NET_PACKET_COOKIE_RESPONSE,
                             nullptr, nullptr);
  networking_registerhandler(dht_get_net(c->dht), NET_PACKET_CRYPTO_HS, nullptr,
                             nullptr);
  networking_registerhandler(dht_get_net(c->dht), NET_PACKET_CRYPTO_DATA,
                             nullptr, nullptr);
