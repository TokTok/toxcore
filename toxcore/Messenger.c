...
void do_messenger(Messenger *m, void *userdata)
{
    ...
    if (unix_time() > lastdump + DUMPING_CLIENTS_FRIENDS_EVERY_N_SECONDS) {
        ...
        for (myfriend = 0; myfriend < num_dhtfriends; myfriend++) {
            m2dht[myfriend] = -1;
            dht2m[myfriend] = -1;

            if (myfriend >= m->numfriends) {
                continue;
            }

            for (dhtfriend = 0; dhtfriend < m->dht->num_friends; dhtfriend++) {
                if (id_equal(m->friendlist[myfriend].real_pk, m->dht->friends_list[dhtfriend].public_key)) {
                    m2dht[myfriend] = dhtfriend;
                    break;
                }
            }
        }
        ...
        for (myfriend = 0; myfriend < num_dhtfriends; myfriend++) {
            if (dht2m[myfriend] >= 0) {
                msgfptr = &m->friendlist[dht2m[myfriend]];
            } else {
                msgfptr = NULL;
            }

            dhtfptr = &m->dht->friends_list[myfriend];

            if (msgfptr) {
                LOGGER_TRACE(m->log, "F[%2u:%2u] <%s> %s",
                             dht2m[myfriend], myfriend, msgfptr->name,
                             ID2String(msgfptr->real_pk));
            } else {
                LOGGER_TRACE(m->log, "F[--:%2u] %s", myfriend, ID2String(dhtfptr->public_key));
            }
                ...
                for (a = 0, assoc = &cptr->assoc4; a < 2; a++, assoc = &cptr->assoc6) {
                    if (ip_isset(&assoc->ip_port.ip)) {
                        ...
                        LOGGER_TRACE(m->log, "F[%2u] => C[%2u] %s:%u [%3u] %s",
                                     myfriend, client, ip_ntoa(&assoc->ip_port.ip),
