 ...
 */
void m_callback_conversation_invite(Messenger *m, void (*function)(Messenger *m, uint32_t, const uint8_t *, uint16_t,
                                    void *))
{
    m->conversation_invite = function;
}
...
static int handle_packet(void *object, int i, const uint8_t *temp, uint16_t len, void *userdata)
{
    ...
    switch (packet_id) {
        ...
        case PACKET_ID_INVITE_CONVERSATION: {
            if (data_length == 0) {
                break;
            }

            if (m->conversation_invite) {
                (*m->conversation_invite)(m, i, data, data_length, userdata);
            }
