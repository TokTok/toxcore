...
{
    size_t size = sizeof(result->uint8);
    assert(size == sizeof(addr->__in6_u.__u6_addr8));
    for (int i = 0; i < size; i++) {
        result->uint8[i] = addr->__in6_u.__u6_addr8[i];
    }
}
...
{
    size_t size = sizeof(ip.uint8);
    assert(size == sizeof(addr->__in6_u.__u6_addr8));
    for (int i = 0; i < size; i++) {
        addr->__in6_u.__u6_addr8[i] = ip.uint8[i];
    }
}
