...
IP4 get_ip4(const struct in_addr *addr)
{
    IP4 result;
...
IP6 get_ip6(const struct in6_addr *addr)
{
    IP6 result;
...
static int receivepacket(Logger *log, tox_sock sock, IP_Port *ip_port, uint8_t *data, uint32_t *length)
{
    ...
    *length = (uint32_t)fail_or_len;
    ...
    if (addr.ss_family == AF_INET) {
        ...
        ip_port->ip.ip4 = get_ip4(&addr_in->sin_addr);
        ip_port->port = addr_in->sin_port;
    } else if (addr.ss_family == AF_INET6) {
        struct sockaddr_in6 *addr_in6 = (struct sockaddr_in6 *)&addr;
        ip_port->ip.family = addr_in6->sin6_family;
        ip_port->ip.ip6 = get_ip6(&addr_in6->sin6_addr);
        ...
        switch (walker->ai_family) {
            ...
            case AF_INET6:
                if (walker->ai_family == family) { /* AF_INET6 requested, done */
                    ...
                    if (walker->ai_addrlen == sizeof(struct sockaddr_in6)) {
                        struct sockaddr_in6 *addr = (struct sockaddr_in6 *)walker->ai_addr;
                        ip6.ip6 = get_ip6(&addr->sin6_addr);
                        result |= TOX_ADDR_RESOLVE_INET6;
                    }
                }
