 ...
 *
 * The output array must be at least `plain_len + TOX_PASS_ENCRYPTION_EXTRA_LENGTH`
 * bytes long. This delegates to tox_pass_key_derive and
 * tox_pass_key_encrypt.
 *
 * @param plain A byte array of length `plain_len`.
 * @param plain_len The length of the plain text array. May be 0.
 * @param passphrase The user-provided password.
 * @param passphrase_len The length of the password.
 * @param cipher The cipher text array to write the encrypted data to.
 *
 ...
 *
 * The output array must be at least `cipher_len - TOX_PASS_ENCRYPTION_EXTRA_LENGTH`
 * bytes long. This delegates to tox_pass_key_decrypt.
 *
 * @param cipher A byte array of length `cipher_len`.
 * @param cipher_len The length of the cipher text array. May be 0.
 * @param passphrase The user-provided password.
 * @param passphrase_len The length of the password.
 * @param plain The plain text array to write the decrypted data to.
 *
 * @return true on success.
 */
bool tox_pass_decrypt(const uint8_t *cipher, size_t cipher_len, const uint8_t *passphrase, size_t passphrase_len,
                      uint8_t *plain, TOX_ERR_DECRYPTION *error);
 ...
 * initialise it, use tox_pass_key_set_key and tox_pass_key_set_salt or one of the derive_*
 * functions below.
 */
...
/**
 * Get the key part of the Tox_Pass_Key. The passed `key` array must have at least
 * TOX_PASS_KEY_LENGTH elements.
 */
 ...
 * TOX_PASS_KEY_LENGTH elements.
 */
void tox_pass_key_set_key(struct Tox_Pass_Key *_key, const uint8_t *key);

/**
 * Get the salt part of the Tox_Pass_Key. The passed `salt` array must have at
 * least TOX_PASS_SALT_LENGTH elements.
 */
void tox_pass_key_get_salt(const struct Tox_Pass_Key *_key, uint8_t *salt);

/**
 * Get the salt part of the Tox_Pass_Key. The passed `salt` array must have at
 * least TOX_PASS_SALT_LENGTH elements.
 */
void tox_pass_key_set_salt(struct Tox_Pass_Key *_key, const uint8_t *salt);
 ...
 * @return true on success.
 */
bool tox_pass_key_encrypt(const struct Tox_Pass_Key *_key, const uint8_t *plain, size_t plain_len, uint8_t *cipher,
 ...
 * @return true on success.
 */
bool tox_pass_key_decrypt(const struct Tox_Pass_Key *_key, const uint8_t *cipher, size_t cipher_len, uint8_t *plain,
 ...
 * @return true on success.
 */
bool tox_get_salt(const uint8_t *cipher, uint8_t *salt, TOX_ERR_GET_SALT *error);
