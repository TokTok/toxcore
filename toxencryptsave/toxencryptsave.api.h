 ...
 *
 * The output array must be at least `plain_len + $PASS_ENCRYPTION_EXTRA_LENGTH`
 ...
 */
static bool pass_encrypt(const uint8_t[plain_len] plain, const uint8_t[passphrase_len] passphrase, uint8_t *cipher)
 ...
 *
 * The output array must be at least `cipher_len - $PASS_ENCRYPTION_EXTRA_LENGTH`
 ...
 * @return true on success.
 */
static bool pass_decrypt(const uint8_t[cipher_len] cipher, const uint8_t[passphrase_len] passphrase, uint8_t *plain)
...
class pass_Key {
   ...
   * initialise it, use ${key.set} and ${salt.set} or one of the derive_*
   * functions below.
   */
  static this new();
  ...
  uint8_t[PASS_KEY_LENGTH] key {
    /**
     * Get the key part of the $this. The passed `key` array must have at least
     * $PASS_KEY_LENGTH elements.
     */
    get();
    /**
     * Get the key part of the $this. The passed `key` array must have at least
  ...
  uint8_t[PASS_SALT_LENGTH] salt {
    /**
     * Get the salt part of the $this. The passed `salt` array must have at
     * least $PASS_SALT_LENGTH elements.
     */
    get();
    /**
     * Get the salt part of the $this. The passed `salt` array must have at
   ...
   * @return true on success.
   */
  const bool encrypt(const uint8_t[plain_len] plain, uint8_t *cipher)
      with error for encryption;
   ...
   * @return true on success.
   */
  const bool decrypt(const uint8_t[cipher_len] cipher, uint8_t *plain)
      with error for decryption;
}
 ...
 * @return true on success.
 */
static bool get_salt(const uint8_t *cipher, uint8_t[PASS_SALT_LENGTH] salt) {
  NULL,
  /**
   * The input data is missing the magic number (i.e. wasn't created by this
   * module, or is corrupted).
